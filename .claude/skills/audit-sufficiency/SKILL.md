...
Conceptual lens - NOT mechanical. Per-step concreteness, atomicity, exact-text checks belong to `audit-haiku-safe`; do not duplicate them here. Focus: invariant violations across sections. Distinct from Self-Critique (structural omissions) and audit-haiku-safe (mechanical safety). See [_shared/audit-stages.md](../_shared/audit-stages.md) for the three-tier focus distinction.
Apply eight lenses in order: assumptions, validation path, test fidelity, orchestration edge cases, freshness, meta, spec-acceptance fidelity, rigour-heuristics-applied. Each lens has a concrete prompt; each finding gets classified Blocker or Not-blocker.
...
- All eight lenses were applied with at least one observation each (or explicit "no findings under this lens").
