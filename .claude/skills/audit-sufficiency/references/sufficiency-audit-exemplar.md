...
We modify execute-plan, retire, write-bus-plan, bus-conventions; create 4 agents and 3 skills; only THEN exercise the pipeline at child 4. If a foundational assumption is wrong, we discover it after ~10 file mutations. Consider a smaller MVP path: ship `audit-haiku-safe` standalone first, validate pinned-model subagents with one tiny agent, defer the full orchestrator. <!-- tally-ok: quoted verbatim from a past review; changing the numbers would falsify the exemplar -->
