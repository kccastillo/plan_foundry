...
# ---------------------------------------------------------------------------
# PLAN-AK6 D6: a target part-way through a failed sync is told so ahead of
# any sha comparison, and before the network call - so the answer survives
# an offline consumer.
# ---------------------------------------------------------------------------


def test_check_current_reports_sync_incomplete_over_a_matching_pin(
    ...
    same_sha = "d" * 40
    _write_pin(target_claude, sha=same_sha, tag="v1.9.0")
    (target_claude / check_current.SYNC_INCOMPLETE_FILENAME).write_text(
        f"previous_sha={'c' * 40}\ntarget_sha={same_sha}\n"
    ...
    def _raise(*args, **kwargs):
        raise AssertionError(
            "fetch_remote_head must not be called when a sync_incomplete "
            "marker is present - the marker is decided before the network "
            "call (D6), which is what this test proves"
        )

    monkeypatch.setattr(check_current, "fetch_remote_head", _raise)

    result = check_current.check(target)
    assert result["status"] == "sync_incomplete"
    assert result["sync_incomplete"]["target_sha"] == same_sha
