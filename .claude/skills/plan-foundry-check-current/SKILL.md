...
`version_compare` (PLAN-AH8) is a separate signal from `status` above - the pre-sync break signal. It compares the pin's own `tag` against the remote's highest parsed tag (integer-tuple, not lexical) so a consumer can tell a breaking sync from a routine one before running it. `unavailable` covers an empty pin tag, an unreachable/failed tag query, or zero parseable remote tags; it is never reported as `current`.

`sync_incomplete` (PLAN-AK6) reports a marker a previous, unfinished `/plan-foundry-sync` run left behind. Decided before the network call, so it survives an offline consumer, and it overrides `status` - a target part-way through a failed sync must not read as `current` or `behind_or_diverged`, whatever its pin says. `null` except when `status == "sync_incomplete"`, in which case it carries `{"previous_sha", "target_sha", "started"}` and the message names the sha the interrupted run was moving to and instructs the operator to run `/plan-foundry-sync` again.
