...
**Naming:** See [references/naming-convention.md](references/naming-convention.md) - `TYPE-NNN_slug.md` format. To compute the next ID for a type, run: `python .claude/skills/write-plan/scripts/next_id.py PLAN` (or ADVICE / RESEARCH).

<essential_principles>
Transcribe plan content accurately. Do not invent, summarise, or modify content.
After writing any PLAN file, regenerate the INDEX projection. Correct PLAN frontmatter is the status update.
...
- Regenerate the INDEX projection after every PLAN file write
</constraints>
