---
name: write-plan
description: 'Plan transcription skill. Writes PLAN files at any point in a plan''s lifecycle - including incremental in-flight writes during ideation, draft updates, and final transcription. Overwrites of existing PLAN files with revised content are permitted. Trigger phrases: "write this plan", "create plan file", "update the plan", "write plan file".'
---
...
**Naming:** See [references/naming-convention.md](references/naming-convention.md) - `TYPE-{ID}_slug.md` format (PLAN's active ID is alpha-alpha-numeric, e.g. `AA0`). To compute the next ID for a type, run: `python .claude/skills/write-plan/scripts/next_id.py PLAN` (or ADVICE / RESEARCH).

<essential_principles>
Transcribe plan content accurately. Do not invent, summarise, or modify content.
Correct PLAN frontmatter is the status update.
For recurring PLAN files: append to the existing file's History table - do not create a new file.
Report back: filename written, ready for next step.
...
ID assignment: call `scripts/next_id.py PLAN` to get the next ID (alpha-alpha-numeric, e.g. `AA0` - see [references/naming-convention.md](references/naming-convention.md)); construct filename as `PLAN-{ID}_{slug}.md`. For RECUR- plans: `PLAN-{ID}_RECUR-{slug}.md`. Never reuse an ID. Never hand-count existing files - always use the script.
</essential_principles>
...
- `target_filename: string` - the PLAN file basename or path under `Workbench/`. Use `TYPE-{ID}_slug.md` format; call `scripts/next_id.py` to get the ID (alpha-alpha-numeric for PLAN, e.g. `AA0`).
...
- The created_month field is set once and never changed
</constraints>
