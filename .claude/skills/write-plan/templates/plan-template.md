...
blocked_by: ""
rollover_count: 0
triggers_plans: []      # Child PLAN filenames for plan-of-plans; may use "[placeholder] <slug>" entries until children draft (see .claude/skills/write-plan/references/plan-of-plans-authoring.md)
...
bundle_version_at_creation: "" # PLAN-AH8. Provenance only, not a guarantee-2 mechanism - populated by write-plan from bundle_copy.bundle_version_string(target_claude) at PLAN-creation time; empty when no pin exists. Never hand-edited.
...
step_remaps: []
# Each entry: {iteration: <int>, map: [{"old": <int>, "new": <int>}, ...]}
# Every ordinal remap applied to this PLAN's ## Steps by the patch-application procedure
# (dispatch.md section 4B). Orchestrator-managed (appended on each renumber); never hand-edited,
# never reset.
...
*Populated by `ideate` at Converge close (decision 15 from the plan-pipeline ideation session) when the PLAN is born from an ideation arc. For ad-hoc PLANs not from `ideate`, leave as "n/a - ad-hoc PLAN, not produced via ideate" or list any decisions you would want a future Human reviewer to know about.*

**Already locked** (Human proposed/affirmed during ideation, so they require no answer at the design-review checkpoint):
- 

**Mechanically forced** (no meaningful alternative, because each one is a downstream consequence of a locked decision):
- 

**Real judgement calls** (genuinely have alternatives the Human might prefer, so they are surfaced for design-review):
- 
...
Mark [blocked-on-input] for steps waiting on an input file (note which file - the file is created via `write-input`).]
...
- [ ] [Subjective item - surfaced for Human eyeball, with no auto-fail]
      `verify: human`
