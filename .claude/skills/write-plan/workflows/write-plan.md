...
Step 1: Receive plan content:
  ...
  - Target filename. If the dispatching orchestrator supplied `target_filename` (SKILL.md `<inputs>`), use it as given - do not re-derive. Otherwise call `scripts/next_id.py PLAN` and construct `PLAN-{NNN}_{slug}.md`. Either way, re-check for a collision (glob `Workbench/` and `Retired/` for the exact ID) immediately before writing: two agents can be authoring PLANs concurrently, and an ID that was free when chosen may not still be free when this step runs.
