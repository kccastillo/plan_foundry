...
Step 1: Receive plan content:
  ...
  - Target filename. If the dispatching orchestrator supplied `target_filename` (SKILL.md `<inputs>`), use it as given - do not re-derive. Otherwise call `scripts/next_id.py PLAN` and construct `PLAN-{ID}_{slug}.md` (ID is alpha-alpha-numeric, e.g. `AA0`). Either way, re-check for a collision (glob `Workbench/` and `Retired/` for the exact ID) immediately before writing: two agents can be authoring PLANs concurrently, and an ID that was free when chosen may not still be free when this step runs.

  **Orchestrator note (not part of this skill's own procedure):** when dispatching more than one plan-writer concurrently, pre-allocate each PLAN ID sequentially in the parent session (repeated `next_id.py PLAN` calls, one per dispatch, before firing any of them) and pass each as `target_filename`. This removes the race entirely rather than relying on each agent's collision re-check to catch it after the fact.
