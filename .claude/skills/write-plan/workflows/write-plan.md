...
Step 3: Regenerate the INDEX projection.
  `Workbench/INDEX.md` and `.index.json` are the sole state surface, projected
  from PLAN frontmatter. Writing the PLAN file with correct frontmatter *is*
  the status update. Regenerate the projection: `Skill("update-workbench-index")`.
