...
**D3 Numeric-Historical-Frozen.** Old `PLAN-NNN` IDs in `Retired/` and the LOG are historical-only and frozen. They are not migrated. Reference them exactly as the LOG records them. When a `PLAN-NNN` collides with an AA-form ID's predecessor (e.g. PLAN-031 renamed to PLAN-AA9), reference the active PLAN by its AA-form ID and disambiguate by appending "(formerly PLAN-NNN)" in narrative prose.

**ADVICE / RESEARCH asymmetry.** ADVICE-NNN and RESEARCH-NNN allocators stay numeric. They never had the collision problem (retire bug only bit PLAN files via plan-retirer). Documented asymmetry — justified by history, not principle.

**Allocator.** `.claude/skills/write-plan/scripts/next_id.py` is the canonical allocator. Source of truth (post-PLAN-AB9, 2026-05-23): filesystem-primary — Workbench/ + Retired/ (recursive) are authoritative for "used IDs". LOG-scan is retained as a non-authoritative defence-in-depth fallback for any pre-D2-A archive state. Warn-and-union semantics preserved. No cache. Read-only contract — not idempotent across calls. See PLAN-AA1's design lineage (research-validated leans across event-sourcing canonical pattern, Postgres `nextval` template, Jira/Linear/GitHub ID conventions); inversion of FS-vs-LOG precedence per PLAN-AB9 D2.
...
### Slim-LOG contract (per PLAN-AB9, effective 2026-06-01)

From the June 2026 LOG forward, the monthly LOG is **slim**: it carries only the surfaces that have no other natural home in the foundry. The Status Table is dropped from active maintenance — INDEX (live projection of PLAN frontmatter) is now the sole canonical surface for "is PLAN X done?" — and end-of-day LOG row maintenance ends.

Sections retained in a slim LOG:

- **Recurring Task Tracker** — RECUR PLANs, their cadence, last-done, next-due. Stays LOG-side because INDEX is regenerated on every commit and has no `last-done` write-back path (per AB9 D4 and ADVICE-006 §3).
- **Lessons Learned** — per-month curated lessons with `[category] (src: <source>; recurrences: <N>)` rows. Feeds the maintain-claude-md T06 promotion hook; the monthly grouping is meaningful (per AB9 D5).
- **Context Inputs** — per-month list of ADVICE/RESEARCH files created and which PLANs they advise (per AB9 D5).
- **Rollover / Close marker** — `status: open` / `status: closed` frontmatter; month-end rollover transfers non-terminal artefacts forward.

Sections **dropped** from new slim LOGs:

- **Status Table** — INDEX is canonical; PLAN frontmatter `status` is the sole source of truth. Per AB9 D3.
- **Notes column** — narrative lives in each PLAN's Executor Notes / frontmatter (per AB9 D7).

The allocator (`next_id.py`) now treats the filesystem (Workbench/ + Retired/) as authoritative for used PLAN IDs, with LOG-scan retained only as a non-authoritative defence-in-depth fallback for any pre-D2-A archive state (per AB9 D2). This is feasible because PLAN-AD0 D2-A (2026-05-22) flipped `Retired/` to a tracked directory.

### LOG format transition (2026-06-01)

The May 2026 LOG (`Workbench/202605010000_LOG_202605.md`) stays in **fat** shape — Status Table fully maintained through end-of-month for audit-trail continuity. The June 2026 LOG and every LOG thereafter starts **slim**, per the contract above. The retire skill and plan-pipeline orchestrator both conditionalise their LOG-update steps on `log_month ≤ 202605`:

- `log_month` ≤ `202605`: legacy fat-LOG behaviour — Status Table rows are updated on retire and on phase boundaries.
- `log_month` ≥ `202606`: slim-LOG behaviour — Status Table updates are no-ops; INDEX projects from frontmatter.

The YYYYMM comparison is performed as a lexicographic string-compare, which sorts correctly across the year-end boundary.

This boundary is **D6** of PLAN-AB9 (cutover date locked overnight 2026-05-23). No back-migration of historical LOGs — pre-May-2026 LOGs retain their full Status Tables as historical records.
...
LOG Status Table "Status" column mirrors the plan's frontmatter `status` at end of day for LOGs whose `log_month` ≤ `202605` (fat-LOG era). For `log_month` ≥ `202606` the Status Table is absent and PLAN frontmatter `status` (projected by INDEX) is canonical — see the Slim-LOG contract above. Rollover treats `done`, `cancelled`, `closed` as complete; everything else rolls over.
...
**Project default:** `manual`. The earlier `.claude-plugin/marketplace.json` project-default layer was removed by PLAN-AC4 — a git-history audit confirmed the field had never been set to anything other than `"manual"` in tree, so the reader was collapsed into the hard-coded default. Set `push_policy: auto` per-PLAN for individual plans that need immediate remote propagation.
...
- `audit_state` — durable audit loop state; includes `sufficiency_iterations`, `plan_safety_iterations`, `auto_fix_iterations`, `last_stage`, `last_outcome`, `last_audit_commit`, `preferred_model_override`.
...
A **preliminary holding-PLAN** (also called a holding-PLAN) is a PLAN file created before ideation has begun — used to park ADVICE/RESEARCH inputs and frame the open questions that ideation will eventually answer.
...
- `linked_inputs: [...]` — pre-populated with the RESEARCH and ADVICE files that motivated the holding-PLAN. These are the primary inputs ideation will consume.
...
**Use case:** surfaces a durable, version-controlled destination for ADVICE/RESEARCH inputs that are not yet ready to walk the full ideate cadence. Allows a batch of inputs to be triaged into action-ready holding-PLANs before committing to spec work on any one of them.

**Pattern origin:** Emerged empirically during the PLAN-AA0 plan-of-plans session (2026-05-16), where six holding-PLANs were created without dedicated tooling. Canonised in PLAN-AB0 per retrospective finding F3 (Reeve retrospective, 2026-05-16, `Workbench/202605160001_RESEARCH_plan-foundry-retrospective.md`).
...
If any discrete deliverable count (MCP tools, schema tables, context-window slices) exceeds **0.8×** a threshold in `.claude/skills/_shared/capacity-thresholds.md`, the PLAN's Context section MUST acknowledge the brushing and note that a research bot was dispatched to confirm the threshold's current relevance.

**Origin:** Plan B initially specced 49 tools against a ~50-tool MCP degradation threshold; the brushing was caught only by a self-critique research bot, not pre-empted by the spec itself (2026-05-16 hiccup log §H2).
...
**Origin:** Plan B's spec stated "tests are sync" without documenting the async/sync boundary; the executor had to make a judgement call that the spec should have pre-empted (2026-05-16 hiccup log §H4).
...
**Origin:** Plan A's executor produced an ADVICE doc with L-tier notes integrated inline rather than under a dedicated `## Notes` heading, because the spec said "should have a Notes sub-section" rather than specifying the heading literally (2026-05-16 hiccup log §H8).
...
**Source of truth:** `.claude/skills/ideate/workflows/cadence-phases.md` § Phase 2 (Survey), § Phase 5 (Self-Critique), § Phase 7 (Cross-Spec-Reconcile).
**Structural tests:** `.claude/skills/ideate/lib/test_cadence_structure.py` (7 tests; run via `python -m pytest`).
