...
- `<TYPE>` - a live item token (uppercase): `PLAN` or `INPUT`. `ADVICE` and `RESEARCH` are grandfathered - read them, never generate them.
...
**ADVICE and RESEARCH - `\d{3,4}`, grandfathered.** Both kinds were collapsed into `INPUT` on 2026-08-03 (PLAN-AJ3). Existing files under either token stay where they are and stay readable. `next_id.py` keeps both tokens because the allocator derives from the filesystem and stays correct for that space, but nothing calls it for an input any more.

The canonical allocator is `.claude/skills/write-plan/scripts/next_id.py`. It issues AA-form IDs for PLAN. Source of truth: the filesystem (Workbench/ + Retired/), and only the filesystem. See PLAN-AA1's Decision Classification for design lineage.
...
| `RESEARCH` | Grandfathered. Read-only; superseded by `INPUT` on 2026-08-03 |
| `ADVICE` | Grandfathered. Read-only; superseded by `INPUT` on 2026-08-03 |
