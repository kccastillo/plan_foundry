...
**Execution floor is Sonnet (recalibrated 2026-07-04).** The default executor runs Sonnet 5; only `opus` escalates. Empty/`haiku` → Sonnet `plan-executor` (size **S**), `sonnet` → the identical `plan-executor-sonnet` (size **M**), `opus` → `plan-executor-opus` (size **L**). See the phase-state-machine executor-tier table and [../../_shared/plan-safe.md](../../_shared/plan-safe.md) § Executor t-shirt sizing. When a PLAN carries a `size:` field it must agree with `assigned_to:` per that mapping.
