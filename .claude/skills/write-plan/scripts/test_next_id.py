...
Covers:
  ...
  13-17. Verbose/by-source partition and the --explain stderr contract.
  18-20. A burn tombstone takes its ID out of circulation and is labelled Burned.

The list above names what is covered; it carries no tally, because a count in a
docstring is wrong the first time someone appends a test and does not notice.
...
glob_burned_ids = _mod.glob_burned_ids
...
# ---------------------------------------------------------------------------
# Test 18 - a burn tombstone takes its ID out of circulation
# ---------------------------------------------------------------------------

def test_18_burned_id_is_never_allocated():
    """
    The failure this guards against: an ID retired without ever owning a PLAN
    file is invisible to a filename-derived allocator, so it gets handed out a
    second time and two unrelated pieces of work end up sharing an identifier.
    This bit on 2026-07-31, when the allocator offered AI6 after AI6 had been
    burned on 2026-07-30 and named in live handoff files.

    A tombstone closes it with no allocator special case: the file matches the
    ordinary PLAN-*.md scan, so the ID is simply in use.
    """
    with tempfile.TemporaryDirectory() as tmp:
        repo, wb, rt = _setup_repo(tmp)

        (wb / "PLAN-AA0_active.md").write_text(_PLAN_STUB)
        (rt / "PLAN-AA1_BURNED_reverted-work.md").write_text(_PLAN_STUB)

        used = compute_used_ids(repo)
        assert "AA1" in used, (
            f"Burned AA1 must count as used; used={sorted(used)}"
        )
        assert next_aa_id(used) == "AA2", (
            f"Expected AA2 (AA1 burned), got {next_aa_id(used)}"
        )


def test_19_glob_burned_ids_selects_only_tombstones():
    """
    glob_burned_ids reports tombstones and nothing else, from either directory.
    It never widens the used set - it only supplies the `--explain` label - so a
    false positive here would mislabel a live PLAN rather than lose an ID.
    """
    with tempfile.TemporaryDirectory() as tmp:
        repo, wb, rt = _setup_repo(tmp)

        (wb / "PLAN-AA0_active.md").write_text(_PLAN_STUB)
        (rt / "PLAN-AA1_BURNED_reverted.md").write_text(_PLAN_STUB)
        month_dir = rt / "202604"
        month_dir.mkdir()
        (month_dir / "PLAN-AB0_retired.md").write_text(_PLAN_STUB)
        (month_dir / "PLAN-AB1_BURNED_abandoned.md").write_text(_PLAN_STUB)
        # A slug that merely contains the word must not register as a marker:
        # the marker is the underscore-delimited token, not the substring.
        (wb / "PLAN-AC0_burnedown-chart-tooling.md").write_text(_PLAN_STUB)

        assert glob_burned_ids(repo) == {"AA1", "AB1"}, (
            f"Unexpected burned set: {sorted(glob_burned_ids(repo))}"
        )


def test_20_explain_labels_a_burned_id_as_burned():
    """
    An operator reading the --explain table has to be able to see *why* an ID is
    skipped. A burned ID labelled `Retired` looks like a PLAN that ran, which
    invites someone to go looking for a file that was never written.
    """
    import subprocess
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Copy the script in so find_repo_root (which walks up from __file__'s
        # parent, not from cwd) lands on the synthetic repo rather than this one.
        import shutil
        script = repo / "next_id.py"
        shutil.copy(SCRIPT_DIR / "next_id.py", script)
        result = subprocess.run(
        ...
        assert result.stdout.strip() == "AA2", (
            f"Expected stdout 'AA2', got {result.stdout!r}"
        )
        labelled = dict(
            line.split("\t", 1)
        ...
        assert labelled.get("AA1") == "Burned", (
            f"Expected AA1 labelled Burned; got {labelled!r}"
        )
        assert labelled.get("AA0") == "Workbench", (
            f"Expected AA0 labelled Workbench; got {labelled!r}"
        )
