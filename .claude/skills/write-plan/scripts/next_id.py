...
ADVICE and RESEARCH were collapsed into the INPUT kind on 2026-08-03 (PLAN-AJ3 D1).
An input carries a datetime in its filename and needs no allocation, so nothing
calls this script for one. Both tokens are kept here because the allocator derives
from the filesystem and stays correct for the grandfathered space (PLAN-AJ3 D5).
Do not use them to name a new file.
