...
where TYPE is one of: PLAN, ADVICE, RESEARCH
    ...
    `<ID>	<source>` where `<source>` is `Workbench` or `Retired`. Lines are
    emitted in lexicographic order by ID and are followed by a single trailing
    blank line. The stdout next-ID print and exit code are unchanged.
