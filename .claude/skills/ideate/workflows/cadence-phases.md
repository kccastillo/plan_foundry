...
**R1 — Reusable-asset surfacing at Clarify-open (per PLAN-AC2 D5b + AC2b D6b).** After reading the substrate and before formulating the first clarifying question, the orchestrator SHOULD:
1. Write the user's seed text (trigger phrase or initial brief) to a temp file (e.g. `.heartbeat/.ideate-seed-tmp.txt`).
2. Invoke `python .claude/skills/_shared/list_reusable_assets.py query --seed-file <path> --format markdown`.
3. If stdout is non-empty (matches found), prepend the stdout block to the orchestrator's response above the clarifying questions, under heading `### Reusable-asset check (auto-surfaced from registry)`.
4. If stdout is empty (zero matches) AND `references/.registry.json` exists and contains ≥1 asset, the orchestrator MAY ask the operator one line: "Any topic-tags you want me to check the asset registry for? (comma-separated, or skip)". If operator provides tags, re-invoke with `--tags`. If operator skips, proceed silently.
5. If stdout is empty AND the registry is empty or absent, surface nothing.

Surfacing is pointers-only (per ADVICE-011 C1) and read-only (per C4 — does not update `last_consulted`). This step is conditional on the plan_foundry bundle being installed; if `list_reusable_assets.py` is absent or raises an error, skip silently. Silent-on-zero per AC2b D5b — do not emit an empty table or heading.
...
4. **Output of 2.A:** an internal options-with-reasons list per cluster, classified as Real-judgement-call, Mechanically-forced, or Already-locked (per decision 15). This feeds 2.B (research trigger) and 2.C (presentation).
...
2. **Auto-dispatch threshold (≥2 Real-judgement-call clusters):** If 2 or more Real-judgement-call clusters were identified, the orchestrator auto-dispatches one `survey-researcher` subagent per cluster (`subagent_type: survey-researcher`). The `survey-researcher` agent carries the Sonnet model pin — dispatching by name enforces the tier rather than relying on the session model. Each agent:
...
6. **Research output destinations (both, always):**
   - RESEARCH file on disk via `Skill("write-input")` — reusable across PLANs, persistent, referenced in `linked_inputs`.
   - Inline summary appended to the Converge close — orchestrator's working context at decision time.

7. **Mandatory write-input before phase advance (per PLAN-AB8, applies to ANY phase that dispatches research bots — Clarify expand-research, Survey 2.B, Self-Critique research dispatch, Cross-Spec-Reconcile research dispatch):** before advancing past the dispatching phase, the orchestrator MUST persist each research-bot return (or each logical cluster of returns) as a Workbench RESEARCH file via `Skill("write-input")`. The PLAN's `linked_inputs` frontmatter array MUST reference the new RESEARCH filename(s). Memory-file synthesis, inline Converge summaries, and conversation-transcript references do NOT substitute for the on-disk Workbench artefact — compaction breaks the audit trail. Naming: `RESEARCH-NNN_topic-slug.md` (new AA-era convention; legacy timestamp-prefix form is grandfathered). File content: verbatim bot returns + ≤200-word synthesis at the top. Forensic backstop: audit-sufficiency S701 catches dispatch-without-RESEARCH if this procedural step is bypassed.
...
2. **Tradeoff table with triage column (F7).** One row per option per cluster. Required columns:
   ...
   - **Decision-tier:** `locked` / `forced` / `judgement` (per decision 15: Already-locked / Mechanically-forced / Real-judgement-call).
...
**R3 — Reusable-asset surfacing at Spec-Draft entry (per PLAN-AC2 D5b + AC2b D4b + D6b).** Immediately after plan-writer exits with the written PLAN file on disk, before formulating the Spec-Draft prompt, the orchestrator SHOULD:
1. Read the just-written PLAN's `tags:` frontmatter field.
2. If tags are non-empty, invoke `python .claude/skills/_shared/list_reusable_assets.py query --tags "<comma-joined-tags>" --format markdown`.
3. If stdout is non-empty (matches found), prepend it to the orchestrator's Spec-Draft prompt under heading `### Reusable-asset check (auto-surfaced from registry)`.
4. Silent-on-zero per AC2b D5b — if stdout is empty, prepend nothing (no heading, no empty table).
5. The operator MAY consult any surfaced asset by reading it explicitly. Consultation is what AC2c will eventually stamp as `last_consulted`; AC2b's surfacing itself does not write `last_consulted` (pure-read per ADVICE-011 C4).

Surfacing is conditional on the plan_foundry bundle being installed and `list_reusable_assets.py` being present; skip silently on absence or error. Note: with only the initial 4-helper corpus, R3 will typically return zero matches (S1 — empirical reality at AC2b ship time); this is correct and expected. R3 becomes more useful as the asset corpus grows.
...
2. **Findings table with triage column (F7).** Required columns:
   ...
   - **Decision-tier:** `locked` / `forced` / `judgement` — how much Human attention this finding requires. `judgement` findings are the ones the Human needs to decide on; `forced` findings have one obvious fix; `locked` findings have already been addressed in the spec.
...
2. **Conflict table with triage column (F7).** Required columns:
   ...
   - **Decision-tier:** `locked` / `forced` / `judgement`. `forced` conflicts have one obvious fix and the orchestrator can apply it without prompting; `judgement` conflicts require Human input. The triage column lets the orchestrator apply `forced` fixes autonomously before surfacing only `judgement`-tier conflicts to the Human.
