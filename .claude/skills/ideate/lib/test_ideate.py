...
# Only this lib dir goes on sys.path, so `import state` resolves.
#
# The severity-surface lib (.claude/skills/plan-pipeline/lib) is deliberately
# NOT seeded here. It used to be, and that is what hid a real defect: until
# 2026-07-28 render_critique.py resolved its own severity-surface path to the
# plugin-marketplace layout abandoned in PLAN-AC3, so the module raised
# ImportError on every real invocation - while this suite stayed green,
# because the path seeded on line 29 made the import succeed before the
# module's own broken resolution was ever consulted. The test knew the right
# path and the production module did not.
#
# Leaving this out means render_critique must resolve its own dependency, so
# the import below exercises the real path and the defect cannot silently
# return. Do not re-add plan-pipeline/lib here.
