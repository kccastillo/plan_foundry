...
Reference:
    .claude/skills/plan-pipeline/lib/render_prompts.py
    .claude/skills/plan-pipeline/lib/parse_replies.py
    .claude/skills/ideate/references/critique-schema.md
"""
...
# ---------------------------------------------------------------------------
# Library import - severity-surface modules live in the plan-pipeline skill.
#
# Corrected 2026-07-28. This resolved to `<repo>/plan-foundry-core/skills/
# plan-pipeline/lib` - the plugin-marketplace layout abandoned in PLAN-AC3 in
# favour of real `.claude/skills/` directories. The directory has not existed
# since, so every import raised, and the error message blamed a missing bundle
# install rather than naming the stale path. Skills are siblings, so resolve
# from parents[2] (`.claude/skills`) rather than walking out to the repo root.
# ---------------------------------------------------------------------------

_SEVERITY_SURFACE_LIB = Path(__file__).resolve().parents[2] / "plan-pipeline" / "lib"
...
try:
    ...
    raise ImportError(
        ...
        "Expected it alongside this skill under .claude/skills/plan-pipeline/lib/. "
        f"Original error: {e}"
    ) from e
