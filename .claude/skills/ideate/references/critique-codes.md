...
**When to use:** A step says `import from old/lib/bar.py` but that file was moved; a context section references a PLAN that has since been retired; a step references a config file that has been superseded.
...
```json
{
  ...
  "issue": "Step 11 modifies build_index.py to add stuck_ideation alert. The in-flight PLAN PLAN-AA0_index-v2-alerts.md also modifies compute_alerts() to add a different alert. If both PLANs execute sequentially, the second will overwrite changes made by the first.",
  "suggested_fix": "Coordinate with PLAN-AA0_index-v2-alerts.md - either merge the alert additions into one PLAN, or add a cross-reference and explicit sequencing constraint between them.",
  "fingerprint": ""
}
```
