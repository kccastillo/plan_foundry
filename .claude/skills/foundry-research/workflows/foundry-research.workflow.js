...
// Reference run (no target) ≈ 1 + 5 angles + 12 fetch + 20 claims×3 votes + 1 ≈ 79 agents.
...
log("Budget: " + (HAVE_BUDGET ? (Math.round(budget.total / 1000) + "k target → scale " + SCALE.toFixed(2)) : "no target set — default sizing") +
    " · caps: " + ANGLES_TARGET + " angles / " + MAX_FETCH + " fetch / " + MAX_VERIFY_CLAIMS + " claims × " + VOTES_PER_CLAIM + " votes")
...
// the verify fan-out (claims × votes, on haiku) plus the opus synthesis still
