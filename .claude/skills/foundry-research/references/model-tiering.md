...
**Corrected 2026-07-27 by the W2.2 first-instance check (RESEARCH-012).** This
section previously stated that the Workflow harness exposed `model:` but no effort
knob, and left a conditional: *"If a future harness version exposes effort, set
medium for retrieval roles and high for synthesis."* **That condition has since been
met.** The Workflow `agent()` options now expose `effort` with the full
`low | medium | high | xhigh | max` ladder, and `opts.effort` is documented as
inheriting the session effort when omitted.

The conditional is therefore armed rather than deferred. Set effort per role:
