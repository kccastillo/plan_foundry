...
Build one skill per run. If the request describes several, build the first and say
so.
...
This skill does not restate the standard. Where this file and the standard appear to
disagree, the standard wins and this file is the defect.

[`../_shared/harness-contract.md`](../_shared/harness-contract.md) holds the current
description cap and the listing budget. Read both values from there rather than from
memory, because circulating community figures for both are superseded and the register
carries the version each value was observed against.
...
- Do not restate the standard inside the new skill. Link to the standard instead.
...
1. Settle what the standard requires a SKILL.md to say: the objective, the
   constraints, and what done looks like. Write them down before creating a directory.
2. Pick the name, using lowercase letters, digits and hyphens. Check the name against
   the existing corpus for a near-collision as well as an exact one, because two
   plausibly-matching descriptions are resolved by model judgement and nothing else.
...
[workflows/prove-triggering.md](workflows/prove-triggering.md).

The proof needs a set of prompts that must fire the skill and a set of near-miss
prompts that must not, each run more than once, and the negative set passes by not
firing. Report both rates. A description that fires on everything passes the positive
set and is worse than the description it replaced.
...
- The combined description length is within the cap in the harness contract, and the
  addition to the listing total has been reported rather than absorbed silently.
...
and the best iteration picked by held-out score. Use `skill-creator` when a
description resists tuning.

Run `skill-creator` from its own repository, and do not copy its code into this
bundle. Its frontmatter validator encodes the narrower platform field set, while this
bundle writes against the wider Claude Code set, so running that validator against a
conforming skill here reports a false failure. The standard records that failure as
expected rather than as a defect to fix.
