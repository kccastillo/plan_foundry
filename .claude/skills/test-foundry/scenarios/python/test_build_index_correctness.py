...
def run() -> dict:
    ...
    # Corrected 2026-07-28. This pointed at `.claude/plan-foundry-core/skills/...`,
    # the plugin-marketplace layout abandoned in PLAN-AC3, so `is_file()` was
    # always false and this scenario returned `skip` on every run since. A
    # 217-line correctness scenario that has never asserted anything reads as
    # coverage while providing none - the same defect as `render_critique.py`.
    build_script = (
        repo_root
        / ".claude"
        / "skills"
    ...
    try:
        ...
        # No LOG fixture. The monthly LOG was dissolved 2026-07-28 and
        # build_index has never read one - it only ever skipped the filename.
