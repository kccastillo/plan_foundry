...
Flags:
    ...
    --output <path>     Write the JSON aggregate to <path> instead of stdout.
                        ...
                        per `_shared/executor-capability-boundary.md` boundary (a).
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Post-PLAN-AC6: resolve repo root from cwd (where the human invoked
    # the harness), not from this script's __file__. The script lives
    # inside the project's own `.claude/` after install - that's a copy
    # of the public bundle, fetched on demand. cwd is what the human ran
    # the harness against. The harness scenarios live at
