...
Exercises plan-foundry behaviour against expected outcomes and emits a portable, sequentially-numbered MD test report at `Workbench/testreports/TESTREPORT-NNN_<slug>.md`. The harness runs these execution tiers:
...
The harness is designed to run in a real consumer (e.g. the Reeve repo) after `init-plan-foundry` has bootstrapped plan_foundry there. The TESTREPORT is the cross-back artefact, so the human copies the TESTREPORT from the consumer back into the plan_foundry_dev repo's `Workbench/testreports/` to inform further development.
...
- The test-foundry skill is present at `.claude/skills/test-foundry/`, in the flat skill layout rather than as a separate plugin.
- For `mode=llm-tier-only`: `Workbench/.python-tier-result.json` exists on disk, pre-run by the caller before this skill is invoked.
...
- `mode=full` - runs Python tier discovery, Python tier execution and the LLM tier, and applies to fresh parent-session invocations.
- `mode=llm-tier-only` - skips Python discovery and reads pre-run Python results from `Workbench/.python-tier-result.json`. An orchestrator that ran the Python tier from an executor, and then handed the LLM tier back to the parent session, uses this mode.
...
- Python tier scenarios expose a synchronous `run() -> dict` and carry no pytest dependency.
