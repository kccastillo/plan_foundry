...
which the author assigns and reuses verbatim on restatement, with no
allocator and no hash, as set out in
`../handoff-next-session/references/claim-carry-gate.md`. A checkable claim
also carries a trailing `check: <shell command>` line, where exit 0 means the
claim holds. A freeform claim carries no such line and is never re-run,
because a freeform claim rests on attestation rather than tree evidence, per
the same reference.

Each function below is a pure operation on already-read text and dicts, with
no git and no file I/O of its own, matching `resume_preflight.py`'s existing
shape so that a target predating this mechanism, or a target with no
in-flight claims at all, degrades to the empty case rather than breaking:
...
next handoff. Step 2.6e of `write-handoff.md` enforces that move, not this
module. The threshold is named once here rather than duplicated at each call
site.
"""
...
def parse_claims(handoff_text):
    """Extract every CLAIM-<id> occurrence inside the named sections.

    Returns {id: {"nickname": str, "prose": str, "check": str or None}}.
    An id appearing outside both named sections is ignored, because claim
    identity is scoped to the sections the gate governs.
    """
    claims = {}
    if not handoff_text:
        return claims
...
def diff_dropped(prior_claims, successor_text):
    ...
    successor_text, meaning present neither as a restatement nor as an
    explicit removal note ("CLAIM-<id> ... removed", case-insensitive).
    """
    if not prior_claims:
        return []
    dropped = []
...
def run_claim_checks(claims, repo_root, timeout=30):
    ...
    Checking fails open, mirroring resume_preflight.py's existing subprocess
    pattern, so an unrunnable or timed-out command is `checked: False` and
    NEVER counts as stale. Only a confirmed non-zero exit marks a claim
    stale. A claim with no `check:` line is `checked: False` with a reason
    naming the D2 freeform scope limit, and is never re-run.
...
def next_baseline(prior_baseline, current_claims):
    ...
    prior_baseline plus one when the id is present there, and otherwise
    start at 1. An id absent from current_claims, whether resolved or
    removed, is dropped from the new baseline entirely, so no entries linger
    for claims no longer carried.
