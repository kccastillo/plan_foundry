...
description: The concurrency-and-tier ladder governing discretionary subagent dispatch. Names what runs without asking, what needs an exception, and what an exception obliges the dispatcher to record. Referenced by CLAUDE.md, the `/send-it` and `/pull-back` commands, init-plan-foundry's operating rules and the handoff template rather than restated in each.
created: 2026-07-31
---
...
Difficulty triggers an exception, not importance. A task being consequential,
consumer-facing or irreversible does not raise its rung. The qualifying
condition is that the work does not fit the rung below, and the dispatcher
states the reason **before** dispatch rather than reconstructing it afterwards.
...
Bare `send it` means `send it 2`. The bare form is a convenience rather than a separate mode, and it never lets the agent choose its own level - see "The number is a ceiling" below.

`stand down` is retained as a synonym for `send it 1` rather than replaced by it, because `stand down` reads as a brake. Both are spoken tokens rather than the `/send-it` and `/pull-back` slash commands, so both still work in a session where project-local commands do not load.

The rung argument raises the dispatch ceiling. Above rung 1 the rung does not change the decision autonomy, which is granted identically at rungs 2, 3 and 4: reaching rung 3 still obliges the pre-dispatch reason under "What an exception obliges" above. The token authorises the rung, but it does not excuse the record.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `break glass: <redirect>` | Carry out `<redirect>` even where the redirect departs from the standard workflow, then record the gesture and what it authorised to disk. |
...
- **Recorded.** The orchestrator writes the gesture and the redirect it
  authorised where the work is recorded, in the same two places an exception goes
  under "What an exception obliges" above: the PLAN's Executor Notes for
  pipeline-adjacent work, the handoff for session-level work. Without a record, each individual departure reads as defensible while the aggregate erodes the workflow. A `break glass` that reached no disk did not happen.
- **One redirect, not a standing mode.** A `send it` rung persists for the session, but `break glass` does not. The token authorises the single action the human names and lapses once that action is done. A run that needs the path set aside twice needs the token twice.

Where a pause halts the loop and waits for the human, `break glass` hands the orchestrator a new direction and the run continues along that direction. The token also draws a line the default reluctance cannot draw: the reluctance is correct until the human types the token and wrong afterwards, so neither party has to litigate which case applies.

`break glass` does not widen what autonomy covers. The carve-outs a grant never reaches - promotion, data deletion, an outward-facing act, raising a bound - stay with the human. `break glass` is one of the ways the human exercises those carve-outs directly, never a way the agent acquires them.
...
A high ceiling makes the following easy to forget:
...
- **A swarm of cheap agents is a real option and an encouraged one.** Mechanical
  ...
  agent because the ceiling allows it is the failure to avoid.
...
This file carries no task-to-model table, by design. The mechanism is a
human-set ceiling with the cheapest capable rung chosen per piece of work
beneath that ceiling. A table would freeze into policy what has to be judged per
stage.

Two facts about the dials hold whatever rung is in force. Haiku 4.5 accepts no
`effort` parameter at all, so passing one is meaningless rather than merely
redundant. The effort floor for a judgement-concentrated call is `high`, and
`high` is where a sweep starts rather than where it sits. The sweep runs
downward, because `low` and `medium` are unusually strong on this model
generation, so what a sweep answers is whether a cheaper effort would have done
the job. The live pin follows that floor: `plan-writer` is set to
`effort: "high"` in
`.claude/skills/plan-pipeline/references/phase-state-machine.md`. Record the
result of a sweep where the work is recorded rather than in a session
transcript.
...
**This paragraph is scoped to the session that received the grant.** A grant
covers that session and ends with it, under "Scope and expiry" below, so a
dispatched subagent holds no grant and inherits no autonomy to take one of these
forks on its own lean. A dispatched `plan-executor` that reaches a `[Human]`
step terminates with `outcome: exception` and hands the fork back, which
`.claude/agents/plan-executor.md` requires of it under its exception conditions.
The parent session is then the party that takes the fork, on its own lean where
the grant is in force. The same split runs one level up at PLAN scope, where a
PLAN carrying `assigned_to: human` is not dispatched to an executor at all and
`scripts/ci/check-human-not-dispatched.py` keeps every dispatch surface saying
so.
...
`agent-<id>.meta.json` file carries `agentType`, `toolUseId` and the tier the
dispatcher requested. The sibling `agent-<id>.jsonl` carries the model that
actually ran, on the `model` field of every assistant message. Joining the two
yields the requested tier against the tier that ran. Concurrency sits in neither
field, and is derived instead from the timestamp span of each subagent's own
`agent-<id>.jsonl` - a dispatch's concurrency is how many spans in the session,
counting its own, overlap that dispatch's span.

`.claude/skills/_shared/dispatch_audit.py` performs that join and that
derivation. It is the audit. Read it rather than asking the dispatcher what it
did.

**The reported rung is an upper bound, not a verdict.** The derivation counts
overlapping spans without regard to tier, so a dispatch's concurrency figure
includes every agent alive alongside it whatever tier that agent ran at. The
ladder above counts concurrency within a tier: rung 4 is more than one Opus at
once, and a rung 3 session may run Haiku alongside its Opus without further
justification. One Opus overlapping one Haiku therefore reports a concurrency
of 2 and a rung of 4, where the ladder places it at rung 3. The ladder is the
authority and the reported rung is the approximation, so check the tiers of the
overlapping spans before reading a reported rung 3 or rung 4 as a breach.

## What is enforced, what is audited, and what is only asked

The ladder is checked three ways, which do not carry the same weight.

**Enforced from disk.** No rule in this file is enforced from disk. The disk-derived bounds this file already points to under "What a grant never covers" above - the pre-human bound in `patch_gate.py`, the audit `MAX_ITERATIONS` ceiling - govern the pipeline's own repair loop. A discretionary dispatch falls outside their scope. Nothing in this file stops a rung 3 dispatch from firing before its reason is written, and nothing stops a ceiling from being taken rather than granted.

**Audited after the fact.** `dispatch_audit.py`, named above under "Verifying compliance", joins each dispatch's subagent record to the model that actually ran, and, where a timestamp span exists, to how many agents in the session, counting the dispatch itself, overlapped that span. That join happens after the session, from disk, and gives a reader the tier and concurrency of every dispatch. `dispatch_audit.py` reports rather than enforces, and its docstring states that nothing exits non-zero because a dispatch looks wrong. A breach therefore becomes visible to whoever reads the report rather than blocked at dispatch time, and it becomes visible as a candidate rather than as a finding, because the reported rung is the upper bound described under "Verifying compliance" above.
...
- The cheapest-capable-rightsized economy, under "The number is a ceiling, not a setpoint" above, asks for the cheapest tier and effort per piece of work, one dial moved at a time, with the effort floor read as the point a downward sweep starts from rather than a setpoint. Tier is visible to `dispatch_audit.py` after the fact. Whether a cheaper tier would have done the job is not, and that is the judgement this rule actually governs.
...
- **Never setting your own ceiling**, named under "The agent never sets its own ceiling" above, is not checked by anything in this file, and closing that gap is out of scope here. A ceiling is set by the human's own session turn, so the authoring party and the bound party differ, and a disk-derived check that reads the grant against the session record could close the gap in a separate PLAN. Every other bound in this file depends on the human's grant being what actually landed, so this rule carries the highest consequence of the three and the least protection.
