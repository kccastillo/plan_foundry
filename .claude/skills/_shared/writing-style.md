---
title: Writing Style
description: How written outputs should be written. Which register applies, the two rules that run before drafting, spelling, dates, authorisation to write, plain language, file stamping, the prose tells to watch for, and the sentence-level rules. Adapted from an ASD-STE100 distillation.
created: 2026-07-27
---
# Writing Style

This file carries the detailed word, verb, sentence and structure rules for the artefact register. The stance governing everything Claude writes, including conversation, is the `How to work with the human` section of the root `CLAUDE.md` and of `.claude/skills/init-plan-foundry/operating-rules.md`.
...
A project may add its own rule at `.claude/writing-style-local.md`. The path sits outside the four bundle-managed directories, so sync and uninstall both leave it alone. A supplement may add a rule or tighten one this file already states. A supplement must not relax, disable or override a rule this file states. When the file is absent, nothing changes, and this file governs alone. `writing_lint.py` reads the supplement's "Additional banned words or phrases" list, when present, and folds each phrase into the mechanical checks below. The judgement-based tells above still need a reader to hold them, supplement or not.

## Which register applies

The register follows the reader, not the medium. Ask whether the reader of this text will have had the conversation that produced it.

- **Yes: the conversation register.** Say what happened, say what is coming, then explain, and let separate moves beat one dense sentence. Signposting is the point here rather than padding. Contractions are allowed and the length caps below do not apply. The rules that still hold are the word and verb rules, one name per thing, and the drafting tells.
- **No, or not certain: the artefact register.** Everything in this file applies.

A draft paragraph inside a chat turn that is headed for a file takes the artefact register even though it is chat, while the sentences around it explaining the draft take the conversation register. When one turn holds both, the fence around the draft is the boundary. A subagent report the parent will relay takes the artefact register, because its reader did not have the run that produced it.

Collapsing the two registers fails in both directions, producing telegraphic chat or a decorated document. Signposting and build-up are not the same move. Signposting tells the reader what is coming; build-up withholds the finding for effect. Conversation wants signposting, and neither register wants build-up.

## Before you draft

These two rules run before any prose exists, which is what separates them from the tells below. A tell inspects a finished sentence, so it can only power a revision pass, and a revision pass converges on compliance rather than on a better first draft. These two make the reasoning produce the material the prose needs, so there is nothing left to paper over. Both leave a trace in the reasoning, which is what makes them checkable afterwards.

1. **Write one line per subject first.** For every subject the piece names: its canonical name, what a cold reader must be told at its first mention, and the verb the domain uses for it. Draft only from those lines. A line you cannot fill is a research gap rather than a writing problem, so read the source, the register or the config before writing the sentence.
2. **Write the load-bearing sentence verbatim first.** Decide what the piece is for, then put that one sentence in the reasoning before drafting around it. Everything else exists to make it land at first reading. Emphasis then gets allocated on purpose, before rhythm has a chance to allocate it.
...
### Hold these while drafting

These fire at the moment a word or a clause is chosen, so they change the first draft.

- **Sentences about the document.** A sentence announcing content instead of giving it, at any scale. Throat-clearing openers: "It is worth noting that", "It is important to understand", "In today's environment". The preview-count opener: "Three things it carries that you will want to know", "There are two reasons for this", "Here is what you need to know". Signposting on a short list the reader can already see: "first, second, finally". The structure rule against marginalia below is the same failure at document scale. The test: does the sentence carry a fact, or describe the document? Delete the second kind and start at the point. In the conversation register this tell does not apply, per Which register applies above.
...
- **Uniform hedging.** Every claim softened with "may", "could", "often", "generally". Say what you mean, or say you are unsure and say why. This is confidence calibration rather than style, so the repair is to settle the claim, not to reword it.
...
### Check these after drafting

These are aggregate or positional properties that only exist once there is a draft. Run them as a pass.
...
- **The plainer restatement.** A dense sentence followed by a clearer version of itself. Delete the first version rather than keeping both.
...
The tells above are judgement. These are mechanical, and they can be checked. `.claude/skills/_shared/writing_lint.py` checks four of them (ASCII range, semicolons, sentence-boundary "it", persisted counts) against a path or glob. `scripts/ci/check-writing-lint.py` runs it across every markdown file this section governs, held against a committed baseline so pre-existing findings are backlog rather than a blocker.
...
Two tiers, which set how hard the length caps bind. Which register applies above is a separate question, settled first.
...
**Everything else.** Analysis documents, briefs, summaries, participant-facing prose. Apply the word, verb and structure rules. The length caps do not apply.

### Words

- One name for one thing. Not the pre-fill sheet in one paragraph and the written questions two paragraphs later.
- Reach for the short common word. Start, not commence or initiate. Use, not utilise or leverage. Help, not facilitate. Before, not prior to. After, not subsequent to. About, not regarding or in relation to. Get, not obtain. Show, not demonstrate. Also, not additionally or furthermore. Enough, not sufficient.
- Gloss every identifier, code or coined noun at its first use. "PLAN-AK3, the rolling board orchestrator", not "PLAN-AK3". The same holds for a noun standing in for an event the reader has not met - "the capture", "the leanness pass". Writing the gloss is the default, and omitting it is the decision that needs a reason, because judging what the reader already knows is a judgement made from inside the context that caused the problem.
...
- Pick the verb the domain uses for the subject. A document, requirement, control, file or rule applies, covers, states, requires, defines or excludes. A verb that furniture could equally do is a metaphor standing in for the domain verb you did not have at hand: sits, rests on, lives in, binds, shapes. "The requirement applies to the sensor" beats "the requirement lives at the sensor". When no domain verb comes to mind, you do not yet know how the subject behaves, so go and find out before writing the sentence. One carve-out: `carries` is accepted house usage across this bundle and is not treated as a metaphor.
...
- No phrasal verbs where a plain one exists. Set up, not spin up. Start, not kick off. Ensure, not make sure.
...
- No contractions in artefacts. The conversation register may use them.
...
- Write the requested text and stop. No preamble, no closing summary. Frontmatter, stamp lines and version headers stay. This governs a deliverable, not the conversation register.
...
9. An identifier or coined noun with no gloss at first use -> Add the gloss.
10. A metaphor where a domain verb exists -> Replace it, or go and find out how the subject behaves.
