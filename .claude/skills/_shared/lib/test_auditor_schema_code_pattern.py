"""Tests for the widened findings[].code pattern in auditor-schema-v3.md (PLAN-AG2).
...
_SCHEMA_PATH = pathlib.Path(__file__).resolve().parent.parent / "auditor-schema-v3.md"
...
def _extract_code_pattern() -> str:
    ...
    assert match, "could not find findings[].code pattern in auditor-schema-v3.md"
...
class TestWidenedCodesValidate:
    ...
    def test_capability_boundary_code_validates(self):
        assert CODE_PATTERN.match("EBV001")
...
class TestMalformedCodesRejected:
    ...
    def test_capability_boundary_short_digits_rejected(self):
        assert not CODE_PATTERN.match("EBV1")
