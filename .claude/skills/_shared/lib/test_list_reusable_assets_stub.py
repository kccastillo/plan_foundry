...
def test_all_collected_kinds_are_known():
    assets = collect_assets(_REPO_ROOT)
    # Was `all(kind == "helper")` until PLAN-AH9 (2026-07-28), on the comment
    # "AC2a migrated zero references; only helpers" - true when written.
    #
    # AH9 introduced the first kind: reference asset under _shared/:
    # deprecation-policy.md, the C7 policy. It sits there rather than in
    # references/ because references/ is NOT in promote.sh's ALLOWLIST and so
    # never reaches a consumer, and the deprecation policy is a consumer-facing
    # contract - a consumer has to be able to read how long a deprecated
    # surface survives. Siting it correctly is what broke the old assertion.
    #
    # So the invariant worth holding is that every collected kind is a known
    # one, not that only helpers exist. Do not "restore" the helper-only form:
    # it would force a consumer-facing reference into a directory that does
    # not ship.
    assets_by_kind = {a["kind"] for a in assets}
    assert assets_by_kind <= {"helper", "reference"}, assets_by_kind
    assert "helper" in assets_by_kind
