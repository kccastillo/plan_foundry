...
def test_shipped_ledger_first_entry_wellformed():
    """A-5: the shipped ledger's first entry is well-formed and reads back."""
    ...
    assert entry["path"] == ".claude/skills/_shared/bundle_copy.py::_is_under_known_subskill"
    assert entry["replaced_by"]
    assert entry["note"]
    assert entry["since"]
    assert entry["removed_in"]
