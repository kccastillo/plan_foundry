...
def test_shim_does_not_overwrite_a_real_skill(tmp_path):
    ...
    _write(bundle / "skills" / "old-skill" / "SKILL.md", SHIM_TEXT)
    real = _write(
        target / "skills" / "old-skill" / "SKILL.md",
        "---\nname: old-skill\n---\n\nThe real, working skill body.\n",
    )

    report = bundle_copy.copy_bundle_managed(bundle, target, deprecations=LEDGER)

    assert "skills/old-skill/SKILL.md" in report.shim_skipped
...
def test_shim_over_a_shim_still_copies(tmp_path):
    ...
    assert "skills/old-skill/SKILL.md" in report.files_copied
...
def test_no_ledger_means_plain_copy(tmp_path):
    ...
    target = tmp_path / "target" / ".claude"
    _write(bundle / "skills" / "old-skill" / "SKILL.md", SHIM_TEXT)
    _write(target / "skills" / "old-skill" / "SKILL.md", "richer\n")
...
def test_helper_kind_entry_never_matches_a_copy_target():
    ...
    entries = [
        ...
        {"path": ".claude/skills/old-skill/SKILL.md", "kind": "skill"},
    ]
    assert bundle_copy._shimmed_relpaths(entries) == {"skills/old-skill/SKILL.md"}
