...
deny, allow), the fragment's entries are appended only when they are not
already present, with order preserved and no duplicates. Any missing
keys/objects are created. The hooks block and all other consumer entries are
never touched.
...
merges those entries non-destructively into the consumer's settings.json.
Running the merge twice is a no-op.
...
def _deep_merge_permissions(base: dict, fragment: dict) -> tuple[list[str], list[str]]:
    ...
    fragment's list into the base's list by appending the entries that are
    not already present, which preserves order and adds no duplicates.
    ...
    for key, frag_list in frag_perms.items():
        ...
        if not isinstance(base_list, list):
            # The consumer has a non-list value, so leave that value in place.
            continue
...
def merge_bundle_settings(
    ...
    A missing, empty, or unparseable target is treated as {} and recorded in
    the returned report under "target_precursor" rather than raising.
    ...
    # --- Read fragment (required, so exceptions propagate - a misconfigured bundle) ---
    ...
    # The write happens unconditionally when the target was absent, empty, or
    # unparseable, and when entries were added. The write is skipped only when
    # the target was already correct.
