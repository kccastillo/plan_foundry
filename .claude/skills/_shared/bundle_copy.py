...
Exposes:
  - BUNDLE_MANAGED_DIRS: tuple of the top-level subdirs under .claude
...
This module never deletes from the target. Bundle files that no longer exist
upstream are listed in CopyReport.stale_in_target and left in place, because
explicit cleanup is the user's decision.
...
below - load-bearing, do not reorder):
    ...
    1. read the existing receipt (before the clone, alongside the existing
       read_version_file call in sync.py)
    2. clone
    ...
    9. write the version pin (data pre-resolved via resolve_version, before
       the copy, so the pin's `synced` timestamp and the receipt's sha
...
empty, quarantine never runs on any consumer, and receipt_absent never
reports true. The mechanism would appear to work while doing nothing,
because the report shape is unchanged and the tests of that shape still
pass. Every caller of write_receipt must position the call at the end of
this sequence.
...
docstring and preflight.py's module docstring for why. Do not remove the
duplication.
"""
...
# The literal that preflight.py's `shim_body` places at the start of every
# shim it generates. Matching on this literal answers "is the destination a
# shim this bundle wrote", which is a fact about the bundle's own output
# rather than a guess about which of two files is richer.
...
def _shimmed_relpaths(deprecations) -> set:
    """Return the bundle-relative posix paths carrying a file-addressed
    ledger entry.

    An entry of kind `helper` addresses a `file.py::symbol` and carries no
    file-level path, so no shim is generated for such an entry and such an
    entry never matches a copy target.
    """
    out = set()
...
def copy_bundle_managed(
    ...
    This function never deletes from the target. Returns a CopyReport
    summarising what happened to each file under the bundle-managed dirs.
    ...
    longer matches the destination's current bytes is judged "not ours".
    With force=False that destination is skipped and its display string is
    appended to report.refused_not_ours. With force=True that destination is
    overwritten and its display string is appended to both files_copied and
    forced_overwrites. A destination whose recorded sha256 still matches is
    copied, with nothing new recorded.

    A refused path is deliberately absent from files_copied, files_unchanged,
    project_additions and stale_in_target alike, so the refused path never
    reaches classify_stale and is never misread as gone_upstream, and so a
    caller passing files_copied/files_unchanged to write_receipt cannot
    record a foreign file as this bundle's own. A forced overwrite is
    recorded in files_copied and therefore enters the next receipt, which is
    what makes a resolved conflict clear itself.
    """
...
def _is_under_known_subskill(
    ...
    """Heuristic: a target file is a project addition rather than stale when
    its top-level subdir under the bundle-managed dir is NOT present in the
    bundle. Files under a bundle-known subskill that the bundle dropped are
    stale.
    ...
    Example under skills/:
      ...
      - skills/myproj/X.md -> project addition (bundle does not own skills/myproj/)
    """
    parts = rel.parts
    if len(parts) < 2:
        # A file directly under skills/ or a sibling bundle-managed dir. When
        # the bundle does not ship that file, treat the file as a project
        # addition, because the bundle structure puts everything under a
        # named subdir.
        return True
    top = parts[0]
...
def read_bundle_contract(bundle_root: pathlib.Path) -> dict:
    ...
    PLAN-AH8: this read is deliberately duplicated rather than shared. The
    other copy is inlined in _shared/preflight.py. preflight.py must not
    import this module, because by the time sync's pre-flight runs,
    "bundle_copy" is already bound in sys.modules to the consumer's installed
    copy (see sync.py's _import_local_helpers, called before the clone), so
    any import of bundle_copy from preflight returns pre-this-wave code
    instead of the freshly-cloned bundle's code, and reports no error while
    doing so.

    The same inline-read constraint applies once more in
    ...
    D3) fails CI when this function, preflight.py's inlined copy, and
    installed_bundle_identity disagree on the same fixture input. The
    duplication is intentional and load-bearing, so do not later "simplify"
    the duplicated reads into a shared import.
    """
...
def resolve_version(bundle_root: pathlib.Path) -> dict:
    ...
    to name the sha in an incomplete-sync marker, or in a receipt written
    before the pin - can call resolve_version once and pass the result back
    to write_version_file(..., data=...). Letting resolve_version and
    write_version_file each compute `synced` would produce two timestamps, so
    the pin would disagree with whatever else recorded the first one.
...
def write_version_file(
    ...
    Returns the dict that was written. A supplied `data` is written verbatim,
    because the caller already resolved that dict via
    resolve_version(bundle_root) and this call must not recompute that dict (see
    resolve_version's docstring for why: a second computation would give the
    pin's `synced` timestamp and whatever else the caller recorded first a
    different moment). When `data` is omitted, this call resolves the dict
    itself via resolve_version(bundle_root), which is the exact behaviour
    every caller that predates this parameter already relies on, and the
    existing positional signature and return value are unchanged.
    """
    bundle_root = pathlib.Path(bundle_root)
    target_claude = pathlib.Path(target_claude)
...
def read_version_file(target_claude: pathlib.Path) -> Optional[dict]:
    """Read target/.claude/.plan-foundry-bundle-version and return the parsed
    dict, or None when no pin exists.
    """
    target_claude = pathlib.Path(target_claude)
...
def bundle_version_string(target_claude: pathlib.Path) -> str:
    ...
    target_claude: the pin's tag, falling back to the pin's sha, falling back
    to "" when no pin exists or both fields are empty. Uses read_version_file
...
# check reads ahead of any sha comparison. None of these functions raises,
# because the marker degrades to "absent" on any read failure, the same
# posture read_version_file takes.
# ---------------------------------------------------------------------------
...
def mark_sync_incomplete(
    ...
    The marker's presence means the last sync against this target started
    and did not finish, and the repair is another sync. The marker is not a
    lock and does not block a later run. The write goes through a `.tmp`
    sibling and os.replace, in the pin file's key-value format. Creates
    target_claude when absent. Never raises.
    """
    target_claude = pathlib.Path(target_claude)
...
def read_sync_incomplete(target_claude: pathlib.Path) -> Optional[dict]:
    ...
    because a marker that cannot be parsed is treated the same as no marker.
    """
    target_claude = pathlib.Path(target_claude)
...
# ---------------------------------------------------------------------------
# PLAN-AH7: install receipt, stale classification, quarantine, sweep.
#
# See the module docstring above for the sequence in which these pieces must
# be called. Every piece here is additive, so copy_bundle_managed,
# write_version_file, read_version_file, _is_under_known_subskill and
# CopyReport.stale_in_target are untouched (D7).
# ---------------------------------------------------------------------------
...
def write_receipt(
    ...
    write and the incomplete-sync marker clear that now follow the receipt
    write (PLAN-AK6). Never call write_receipt immediately after the copy,
    because that position is where an earlier revision of this sequence wrote
    the version pin, and where the natural reading would put this call.
    """
    ...
    for rel in sorted(set(relpaths)):
        ...
        try:
            ...
            # The file vanished between the copy and the receipt write, so
            # skip the file rather than raising, because the receipt records
...
def read_receipt(target_claude: pathlib.Path, bundle: str = "") -> Optional[dict]:
    ...
    adoption rule:
      ...
      - No namespaced receipt, a legacy receipt present, and the legacy
        ...
        by this bundle, so the match proves the legacy receipt is ours.
        read_receipt adopts that receipt and returns it with an added key
    ...
    which is what makes the sha comparison meaningful, because once the pin
    is rewritten to the incoming bundle's sha, a legacy receipt from this
    same bundle would no longer match the pin.
    ...
    callers take the "unknown" path, per the bootstrap rule that absent must
    not read as clean. Reads with encoding="utf-8", errors="replace" so
    arbitrary or corrupted content cannot raise a decode error.
    """
    bundle = bundle or "plan_foundry"
    target_claude = pathlib.Path(target_claude)
    try:
...
def _is_stale_by_heuristic(rel: str, bundle_files) -> bool:
    ...
    left untouched per D7, because _is_stale_by_heuristic is a separate,
    additive function.
    """
    parts = pathlib.PurePosixPath(rel).parts
    if len(parts) < 3:
        # A direct child of a bundle-managed dir, such as "agents/x.md". The
        # existing heuristic classifies such a path as a project addition
        # rather than stale.
        return False
...
def classify_stale(bundle_files, target_files, receipt: Optional[dict]) -> ClassifyResult:
    ...
    into the cases the naive stale_in_target heuristic collapses into one
    false-positive-prone "stale" bucket:

      - gone_upstream: recorded in the receipt because this bundle installed
        the file, no longer shipped by the bundle, still present on disk, so
        the bundle genuinely dropped the file. This is the only set
        quarantine() should ever be given.
      - consumer_owned: on disk, absent from both the receipt and the
        bundle, so the file is the consumer's own inside a bundle-owned dir.
        These files are never moved or deleted.
      - unknown: populated only when receipt is None (bootstrap, or a
        corrupt or absent receipt) and holding what the pre-AH7 heuristic
        would have called stale. Report these paths and quarantine nothing,
        because absent must not read as clean (the bootstrap rule).
...
def _force_rmtree(path: pathlib.Path) -> None:
    ...
    Mirrors the identical helper in bundle_fetch.py, run_install.py and
    uninstall.py. Kept local rather than imported, to avoid a cross-module
    dependency for a helper this small, and every copy is intentionally
    identical.
    """
...
def quarantine(target_claude: pathlib.Path, relpaths: list) -> list:
    ...
    This function calls no delete primitive and uses shutil.move only,
    because sweep_quarantine is the one function in this module permitted to
    delete, and only whole aged quarantine directories.
...
def sweep_quarantine(target_claude: pathlib.Path, max_age_days: int = 30) -> list:
    ...
    delete, and sweep_quarantine only ever removes a whole aged quarantine
    directory, never an individual file.

    Directory names that do not parse as the UTC-YYYYMMDDTHHMMSSZ stamp
    written by quarantine() are skipped and never removed, because an
    unparseable name is treated as "not provably aged" rather than as "safe
    to delete".
