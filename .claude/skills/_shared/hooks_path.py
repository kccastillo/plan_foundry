...
Non-clobbering
--------------
A consumer who has already set `core.hooksPath` to a value of their own keeps
that value. `ensure_hooks_path()` reports the conflict rather than overwriting,
because silently redirecting a repo's hooks is a worse failure than leaving the
bundle's hooks unwired.

Ordering constraint
-------------------
The hooks path must not be wired before the line-ending pin is in place. A
CRLF-mangled hook fails on every Windows consumer while exiting 0 on all failure
paths, so a broken hook and a working one are outwardly identical. Wiring the
hooks path first would ship exactly that broken hook. See
_shared/gitattributes_pin.py and FOUNDRYREQ-plan_foundry_dev-20260727-1350.
"""
...
def ensure_hooks_path(target_root: pathlib.Path) -> tuple[str, str]:
    ...
    Returns (status, note):
      ...
      FAIL    - a conflicting consumer-set value is present and is left
                untouched.
    ...
    if rc == 0 and current:
        ...
        return (
            ...
            f"{HOOKS_PATH_VALUE!r} or the hook's behaviour is merged into the existing path.",
        )
