...
Classifies a HANDOFF, FOUNDRYREQ, or OBSERVATION basename into one of these classes:

  conforming       - matches a current new-grammar pattern exactly
  legacy_permitted - a recognised legacy form (valid on read but never a new write target)
...
Reads: this module performs no file I/O, and all logic is pure and
dependency-free.
...
# <origin> is one or more kebab-segments, and the datetime follows the origin directly.
# At least one origin segment is required before the datetime.
...
# A datetime-looking segment would be 8+ digits, so the pattern excludes those.
...
# The scope may itself contain hyphens (e.g. 'dungeon-jaquays'), so the pattern matches
# the scope-plus-timestamp body greedily and requires the last segment to be 12 digits.
...
def classify_artefact_filename(basename: str) -> tuple[str | None, str]:
    ...
    legacy_permitted detection, while the new-grammar conforming checks require
    exact case.
    """
    # ---- HANDOFF family ----
    ...
    if basename.upper().startswith("OBSERVATION-"):
        ...
        # An OBSERVATION- prefix that does not match the known grammar is
        # classified legacy_permitted rather than malformed, because the old
        # skill enforced no strict grammar and existing files would otherwise
