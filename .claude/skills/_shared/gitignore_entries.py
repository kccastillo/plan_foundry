...
does NOT carry "Retired/", because uninstall.py adds "Retired/" to its own
local list (GITIGNORE_BUNDLE_ENTRIES), which keeps the divergence visible
where the entry is used rather than encoding the divergence into the shared
list. Whether
uninstall should stop reversing that entry is a separate question, out of
scope for this PLAN.

REQUIRED_GITIGNORE_ENTRIES includes the paths PLAN-AH7 adds
(.claude/.plan-foundry-bundle-files, .claude/.plan-foundry-quarantine/) and
the incomplete-sync marker PLAN-AK6 adds
(.claude/.plan-foundry-sync-incomplete), so that plan-foundry-sync's
convergence step (sync.py) reaches every already-installed consumer as well
as the fresh installs that go through init-plan-foundry. Adding the paths at
init time alone would leave the entire existing consumer population (the
population this substrate exists for) with these paths untracked and
committable.

PLAN-AK5 adds .claude/.bundle-receipts/ - the namespaced-per-bundle receipt
directory that supersedes .plan-foundry-bundle-files as the read/write
target, while the legacy single file stays listed above and is never
deleted, because a consumer's own legacy receipt, once adopted, is still
...
def filter_tracked(target_root: pathlib.Path, entries) -> tuple:
    ...
    and adding the path to .gitignore untracks that repo's own product as a
    silent side effect of install, which stays invisible until someone reads
    `git status`. An entry with tracked content beneath it is therefore
    dropped and reported (PLAN-AJ6 D4, raised from paper_trail_dev).

    Fail-open: if git is unavailable or the target is not a repo, no entry
    can be shown to be tracked, so every entry is treated as safe. That
...
def ensure_gitignore_entries(
    ...
    because git already tracks content under them. A caller must surface
    `skipped_tracked`, because a dropped entry means the target owns that
    path.
    """
    target_root = pathlib.Path(target_root)
