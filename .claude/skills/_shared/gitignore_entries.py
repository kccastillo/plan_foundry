...
convergence step (sync.py) reaches every already-installed consumer, not
just fresh installs via init-plan-foundry - init-only would leave the
entire existing consumer population (the population this substrate exists
for) with these paths untracked and committable.
...
deleted (a consumer's own legacy receipt, if adopted, still lives there).
"""
