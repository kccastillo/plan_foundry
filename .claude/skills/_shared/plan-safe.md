...
**These four are the recorded instances, not the rule (D8, PLAN-AK1).** The rule is derived from the dispatched agent file's own `skills:` and `disallowedTools:` declarations; a skill absent from the preload list is excluded whether or not it appears below. PLAN-AJ8's `write-skill` and `audit-skills` Steps, as they stood before its plan-safety audit moved those runs to the orchestrator on 2026-08-04, were a crossing none of the four sub-clauses below covers, and the derived check (`audit-haiku-safe` Step 4e) catches that shape anyway. The repaired PLAN no longer contains it, which is why the regression lives in the check's own fixtures rather than in a file that can be fixed out from under it.
