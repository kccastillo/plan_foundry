...
| **L**  | `opus`          | `plan-executor-opus` (**Opus 4.8**)   | Genuinely design-heavy at execution time: a Step needs design decisions that cannot be pre-specified. **Escape hatch — prefer decomposition first (decision 16 anti-monolithic principle).** |
