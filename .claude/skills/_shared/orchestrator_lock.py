...
a short-lived mutex around a single synchronous dispatch walk. The orchestrator
acquires the lock at the top of a state-mutating invocation and releases the lock
when the walk returns control, and a staleness TTL clears the lock after a crashed
session.

D3 - "Per-Repo-Granularity": The lock is keyed on the working tree (one active
orchestrator per repo), NOT per-PLAN. Rationale: orchestrators share one working
tree, so even orchestrators working different PLANs collide on git operations and
intermingle uncommitted work. The architecture already assumes a single orchestrator
("Single conversational re-entry point", SKILL.md essential_principles), and this
module enforces that existing assumption mechanically.

D4 - "TTL-Tradeoff": The lock TTL is generous enough to cover a legitimately long
synchronous walk, because a foreground audit dispatch runs for minutes, and short
enough that a crashed session clears in reasonable time. Default TTL: 3600 seconds
(60 minutes), defined as the module constant LOCK_TTL_SECONDS so the value is
tunable.

DESIGN INSIGHT: the lock does NOT require any persistent session-identity concept.
...
now?". A fresh timestamp plus a held marker answers that question, and the TTL covers
the crash case.
...
returns control, the orchestrator calls release(), so no lock is held while the
executor runs. The executor cannot commit or push, so the irreversible fork vector
cannot originate during that window. The D1 divergence guard (push_guard.py) is the
backstop for the irreversible part.

TTL tuning note: if legitimate synchronous walks routinely exceed 60 minutes,
increase LOCK_TTL_SECONDS. Decrease the value only after confirming that
crash-recovery latency at the new value is acceptable for your workflow cadence.
"""
...
def acquire(repo_root, phase="", plan="", now=None):
    ...
    -------
    dict
        {"acquired": True,  "holder": <new lock dict>} - the lock was written and
            the walk may proceed.
        {"acquired": False, "holder": <existing lock dict>} - another walk holds the
            lock, so the caller MUST refuse to proceed.
    """
    if now is None:
    ...
    if existing is not None and "acquired_at" in existing:
        try:
            ...
            if age_seconds < LOCK_TTL_SECONDS:
                # Another walk holds the lock and the lock is still fresh, so refuse
                return {"acquired": False, "holder": existing}
        except (ValueError, TypeError):
            # A malformed acquired_at is treated as stale, so fall through and
            # write a fresh lock
            pass
...
def release(repo_root):
    ...
    Idempotent: releasing an already absent lock file raises no error.
...
def read_lock(repo_root):
    """Return the current lock dict, or None when the lock file is absent or malformed.
    ...
    -------
    dict or None
        Parsed lock contents, or None when the file does not exist or holds
        invalid JSON.
    """
...
def _read_raw(lock_file):
    """Internal helper: read and parse the lock file, returning a dict or None."""
