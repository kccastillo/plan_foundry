...
call `ensure_gitattributes_pin()`, and neither restates the pin lines.
...
The failure mode is the dangerous kind, because the file looks correct in the
repo, in a diff, and in review, while no ordinary view of the file displays the
CR. The bundle's `commit-msg` hook additionally exits 0 on every failure path by
design so that a hook can never stall a pipeline, which means a broken hook and
a working hook are outwardly identical.

A pin in the *bundle* repo does not travel with copied files, because sync
writes files into the consumer's working tree and the consumer's own
`.gitattributes` governs their checkout. The pin therefore has to be written
into the consumer's repo, which is what `ensure_gitattributes_pin()` does.
...
same pattern is left exactly as the consumer wrote it, whatever value that pin
names.
...
# is left untouched - the bundle does not adjudicate the consumer's choice and
# only ensures that every required pattern is covered.
