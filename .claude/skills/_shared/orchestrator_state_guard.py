...
Note: `status` is intentionally excluded from the guard, because the executor
writes `status` legitimately and `status` is outside the BUG-3 forgery surface.
Guarding `status` would risk reverting a legitimate executor-set value.
...
`target_phase` parameter), the orchestrator MUST set that field in the snapshot dict
to its intended post-dispatch value *before* calling `restore_owned_fields`, so the
restore preserves the directed change while still wiping any other undirected
owned-field write.
...
OWNED_FIELDS = (
    ...
    "verification_state",
    "total_loop_attempts",  # PLAN-AH1 - cross-session IAL guard, managed by the
                            # Routine. NOT zeroed on drafted-revert, unlike
                            # audit_state. Default 0, managed by the orchestrator,
                            # ceiling 10.
)
...
# serialised the sentinel as `!!python/object:builtins.object {}`, which broke the
# next helper's `safe_load`. A namespaced string survives persistence (round-trips
# through JSON/YAML) and reimport (compared by value), so restore always deletes
# an absent field instead of poisoning the file. Compare the sentinel via
# `_is_absent` (==), never via `is`.
...
# popped by restore_owned_fields, so such a value can never reach
# _write_frontmatter. If a non-safe value does reach _write_frontmatter,
# safe_dump remains the loud backstop.
...
def _read_frontmatter_raw(plan_path: Path) -> tuple[str, str, str]:
    """
    Split a PLAN file into (pre_marker, frontmatter_text, body), where pre_marker
    is '' (the empty string before the first ---), frontmatter_text is the text
    between the two --- markers, and body is the rest of the file.
    Raises ValueError when the frontmatter is missing or malformed.
    """
...
def _write_frontmatter(plan_path: Path, fm: dict, body: str) -> None:
    ...
    # safe_dump rather than dump: safe_dump refuses non-standard Python objects, so
    # a stray sentinel or object can never be serialised as a `!!python/object` tag
    # that would break the next safe_load (RESEARCH-005 defence-in-depth). For plain
...
def _load_frontmatter(plan_path: Path) -> tuple[dict, str]:
    """
    Load PLAN frontmatter as a dict and return (fm_dict, body_text), where
...
def snapshot_owned_fields(plan_path: str | Path) -> dict:
    ...
    Returns:
        ...
        frontmatter value, or _ABSENT when the field is missing.

    Raises:
        ValueError: when the file has no frontmatter or malformed frontmatter.
    """
...
def restore_owned_fields(plan_path: str | Path, snapshot: dict) -> None:
    ...
    - When snapshot[key] is _ABSENT, delete the key from frontmatter if present.
    - When snapshot[key] is present but not a YAML-safe owned value (not None/str/int/float/bool/dict/list), delete the key, because such a value is treated as absent or unknown (RESEARCH-005 direction 3).
    ...
    for field in OWNED_FIELDS:
        ...
        # and only genuinely non-serialisable junk is dropped.
