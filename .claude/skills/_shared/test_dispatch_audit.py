...
file reads the real ~/.claude directory - audit_project's claude_projects_root
parameter exists specifically so a test can point it at a tmp_path tree
instead, and every test below does exactly that.
"""
...
def _iso(offset_seconds: float) -> str:
    """A deterministic ISO-8601 UTC timestamp, offset_seconds after a fixed
    base instant - matches the "timestamp" field shape written on every
    real subagent transcript line."""
...
def _timestamp_only_line(timestamp: str) -> str:
    """A line carrying no model - mimics the initiating user turn that
    opens every real subagent transcript, which sets the start of the
    agent's span."""
...
def test_non_pipeline_agent_is_not_exempt(tmp_path):
    ...
    assert rec.rung == "rung 3"  # solo opus, no override, not pipeline-fixed


# ---------------------------------------------------------------------------
# Fable sits outside the ladder
# ---------------------------------------------------------------------------
...
def test_concurrency_overlapping_spans_report_the_overlap(tmp_path):
    session_dir = _make_session(tmp_path)
    subagents = session_dir / "subagents"
    # conc1 runs [0, 20], conc2 runs [5, 25] - they overlap throughout [5, 20].
    _write_meta(
...
def test_concurrency_disjoint_spans_each_report_one(tmp_path):
    session_dir = _make_session(tmp_path)
    subagents = session_dir / "subagents"
    # early runs [0, 10]. late runs [100, 110]. no overlap.
    _write_meta(
...
def test_concurrency_unavailable_for_agent_with_no_timestamps_siblings_still_resolve(tmp_path):
    ...
    # dated1 proves concurrency did derive for the session as a whole.
    assert report.concurrency_available is True
    by_id = {rec.agent_id: rec for rec in report.records}
...
def test_audit_project_never_touches_real_home(tmp_path):
    """claude_projects_root is injected, so this must not read the real
    ~/.claude directory regardless of what is on the machine running it."""
