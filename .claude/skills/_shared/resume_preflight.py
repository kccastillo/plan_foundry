...
20260805-1701):
  D7 - Sibling to the PLAN-drift axis: `expected_claim_checks` (a handoff's merged
    ...
    module - an unrunnable/timed-out check is `checked: False` and never counts as
    drift, only a confirmed non-zero exit does.
  D8 - No git dependency: unlike every other axis, claim checking needs no git
    subprocess at all, so it is computed on BOTH return points of this function -
    the early return taken when `git fetch` fails, and the full-computation return -
    rather than only the git-dependent path. Skipping it on the fetch-failure path
    would silently disable staleness detection in exactly the offline/no-remote
    session shape this axis exists to cover.
"""
...
def check_resume_drift(
    ...
    if not fetch_ok:
        # All git-derived axes are unchecked; still run PLAN snapshot AND the
        # claim-drift axis, which has no git dependency (D8).
