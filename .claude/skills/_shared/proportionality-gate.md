...
- **`plan it`:** gather Objective/Steps/Verification content directly in the
  ...
  runs. The parent session performs both by hand before retiring. For why these
  are orchestrator-only rather than executor-dispatchable, see
  [executor-capability-boundary.md](executor-capability-boundary.md). First, set
