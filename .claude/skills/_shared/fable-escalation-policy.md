...
**Default position: never.** No PLAN, fan-out, or autonomous loop in this repo currently meets the escalation bar below. When deciding a model tier, the answer is almost certainly Opus 5. Only a direct human authorisation overrides this default, governed by the "Human override" section below, and nothing the agent decides for itself reaches Fable.

## The five rules

All five must hold before a Fable request is issued. Any one failing means the answer is no, unless a human waives that rule explicitly for that dispatch under the "Human override" section below.

### 1. Escalation, never selection

Fable is only reachable **after** a recorded Opus 5 attempt at `xhigh` or `max` effort has fallen short, with the shortfall named in writing. Fable is never chosen at planning time as the model for a task.

The difficulty-not-importance test behind this rule is the same one dispatch-authorisation.md applies to its own rung exceptions - see "What an exception obliges" in [dispatch-authorisation.md](dispatch-authorisation.md). Fable's own bar is higher: the recorded attempt must specifically be Opus 5 at `xhigh` or `max` effort, not merely whatever the rung below Fable would have run.

### 2. Never in fan-out

A Fable call is never issued from inside a parallel fan-out - not in a `Workflow` `parallel()` or `pipeline()` stage, not across a set of subagents. Fan-out multiplies whatever runs inside it, and Fable is the most expensive model available. `parallel()` and `pipeline()` are both harness primitives rather than constructs this bundle defines, so the rule covers a stage written in either. What the rule guards against is recorded in [harness-contract.md](harness-contract.md) "Dynamic Workflows script runtime": an `agent()` call that omits `model:` inherits the session model, so a session on Fable spreads Fable across the whole stage.

If a fan-out stage is producing inadequate results, re-scope the stage or raise the Opus 5 effort level rather than swapping the model inside the stage.

### 3. A single call, never a loop

Issue one Fable request per escalation, with no retry loop, no iterate-until-satisfied, and no agentic loop with Fable as the driving model. If one call does not resolve the shortfall named under rule 1, escalate to the human rather than calling again.
...
- **Mid-stream:** partial output already streamed. That partial *is* billed at normal rates. Discard the partial output rather than treating it as a complete answer.
...
Fable authorisation is a human-held key. The `send it` autonomy grant never supplies that key, because a grant of autonomy never covers raising or routing around a bound on the agent's own behaviour, and Fable falls outside the dispatch ladder entirely (see [dispatch-authorisation.md](dispatch-authorisation.md)). No token the agent can be handed, and no judgement the agent makes for itself, reaches Fable.
...
Record the override in the durable record - who authorised it, when, the scope, and the outcome of the single Fable call - in the place the "When the bar is genuinely met" section names. An override that is not written down did not happen, and a later session cannot tell an authorised escalation from a self-selected one.
...
- `claude-mythos-5` - identical capabilities, pricing, and API behaviour, available only to Project Glasswing participants. Use `claude-mythos-5` only if this organisation participates, and `claude-fable-5` otherwise.
