---
asset_id: help-fable-escalation-policy
kind: helper
title: Fable Escalation Policy
topic_tags: [model-selection, escalation, fable, cost-control, dispatch]
description: The single rule set governing when Claude Fable 5 may be dispatched. Escalation only, never selection. Referenced by plan-pipeline dispatch, foundry-research, and autonomous-loop rather than restated in each.
discoverable_via: [plan-pipeline, foundry-research, autonomous-loop, manual]
created: 2026-07-27
last_consulted: ""
consulted_by: []
---
...
**Scope.** This file is the single source of truth for when Claude Fable 5 (`claude-fable-5`) may be dispatched anywhere in plan_foundry. Skills that dispatch models - `plan-pipeline`, `foundry-research`, `autonomous-loop` - reference this file rather than restating its rules. Per PLAN-AD2 W0.3.

**Default position: never.** No PLAN, research fan-out, or autonomous loop in this repo currently meets the escalation bar below. If you are reading this file while deciding a model tier, the answer is almost certainly Opus 5.
...
A Fable call is never issued from inside a parallel fan-out - not in a `foundry-research` role, not in a `Workflow` `parallel()` or `pipeline()` stage, not across a set of subagents. Fan-out multiplies whatever sits inside it, and this is the most expensive model available.
