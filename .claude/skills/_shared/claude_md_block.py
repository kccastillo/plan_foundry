...
The sentinel constants match those in run_install.py verbatim, and build_block emits
the same WARNING comment and wrapping. apply_operating_rules_block reproduces
_step_claude_md branch-for-branch, including returning ("FAIL", ...) without writing
the file when the markers are malformed.
"""
...
def build_block(operating_rules: str) -> str:
    ...
    The result is byte-identical to the inline block construction in
    run_install.py:215-222.
    """
    return (
        f"{SENTINEL_START}\n"
...
def apply_operating_rules_block(
    ...
    Returns (cmd_status, cmd_note) where cmd_status is one of:
        "PASS"    - the file was written, whether created, appended to, or
                    replaced
        "SKIPPED" - the block is already current, so the file is unchanged
        "FAIL"    - the markers are malformed, so the file is NOT written
                    and nothing on disk is destroyed
    """
...
def _sentinel_body(text: str) -> Optional[str]:
    ...
    in `text`, stripped of leading and trailing blank lines. Returns None
    when the markers are absent, duplicated, or malformed (end before
    start).
    """
    start_count = text.count(SENTINEL_START)
    end_count = text.count(SENTINEL_END)
...
def block_change_report(target_root: pathlib.Path, operating_rules: str) -> dict:
    ...
    The comparison uses the body build_block(operating_rules) produces
    between the same markers rather than operating_rules alone, because
    build_block prepends a managed-block WARNING comment that every real
    target's block carries and operating_rules does not, so comparing
    against operating_rules directly would report that comment as removed on
    every sync. This mirrors apply_operating_rules_block's own
    `text[s:e_line_end + 1] == block` disjunct, which exists for the same
    reason.

    The comparison strips trailing whitespace from each line and ignores
    blank lines. Removed and added lines are returned verbatim, in their
    original order, and are never counted or truncated.

    This function never writes. Any OSError yields "unavailable" instead of
    propagating, so this function never raises.
    """
