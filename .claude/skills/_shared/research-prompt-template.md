...
The `survey-researcher` agent (`.claude/agents/survey-researcher.md`) reads this file at Survey phase 2.B and includes every listed sub-question in the prompt it sends. That is the live wiring, shipped under PLAN-AA9 (Reeve F1 research-anchor) and pinned to Sonnet by the retired PLAN-AE7.

Until 2026-08-10 this paragraph described AA9 as unshipped and said its dispatch logic "should" read the file, which sent a first-time reader looking for a mechanism that had already arrived and been superseded by a named agent this same file documents further down.
