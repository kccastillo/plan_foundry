...
The ideate orchestrator reads this file at Survey phase 2.B and includes every listed sub-question verbatim in the `research_brief` it constructs, then dispatches that brief to the `survey-researcher` agent (`.claude/agents/survey-researcher.md`). The agent receives the brief; it does not assemble it. That is the live wiring, shipped under PLAN-AA9 (Reeve F1 research-anchor) and pinned to Sonnet by the retired PLAN-AE7.

**Maintenance rule:** add a sub-question here when a new class of hallucination-from-missing-ground-truth has actually occurred and the occurrence is recorded somewhere a later reader can check it. There is no standing register of these classes to confirm one against, so the new section names its own originating instance and where that instance is written down, the way each section below names its own. Do not add speculative or low-signal sub-questions - each entry must be backed by a concrete instance or a recurring failure pattern.
...
**Originating hiccup:** H5 - Plan B Spec-Draft used `s._tools` (FastMCP private attribute) in two verify/acceptance gates. The research covered FastMCP conceptually, but Spec-Draft jumped to "use FastMCP's tool registry" without checking the public introspection API.
...
**Expected output shape from research bot:**
- Placement: see Output Format below (Sub-question 3's CONFIRMED/REVERSAL line goes first).
...
**Expected output shape from research bot:**
- Placement: see Output Format below (Sub-question 4's Verdict line goes last).
- One sentence maximum
- Cites the single strongest piece of evidence driving the verdict

---

*(Add further sub-questions here as new failure classes accumulate. Each section names its originating instance and where that instance is recorded, then carries the required sub-question text and the expected output shape.)*
