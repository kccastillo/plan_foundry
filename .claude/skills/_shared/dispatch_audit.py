...
This module reports rather than enforces. Nothing here raises the process
exit code because a dispatch looks wrong, since a wrong-looking dispatch is
a finding for the reader to act on rather than a reason to fail a build.
The CLI below always exits 0 on a successful read, whatever the records
show.

This module joins these record shapes on disk:
...
names - is in neither record. This module derives concurrency from each
agent's own <session-dir>/subagents/agent-<id>.jsonl, because every line
the harness writes to a subagent transcript, including the initiating user
turn, carries a top-level "timestamp" field, so an agent's span runs from
its first timestamp to its last. Concurrency for one dispatch is how many
agents in the session - counting the dispatch itself - have a span that
overlaps that dispatch's span. The derivation is coarser than true
wall-clock concurrency, because a span counts any idle gap inside its own
window as running, and an agent that was dispatched but wrote no message
has no timestamp and therefore no span. Such an agent's concurrency is
reported as unavailable rather than as "1", because defaulting to the
flattering value is what this derivation exists to prevent.
...
this module reads are stored outside the repository under the user's home
...
def load_pipeline_fixed_agents(agents_dir: pathlib.Path) -> Dict[str, Optional[str]]:
    ...
    phase state machine dispatches through, under the pipeline-dispatch
    exemption in dispatch-authorisation.md: "the phase state machine and
    agent files fix those tiers". The list is never hard-coded, because
    this function reads the agents directory fresh on every call, so a new
    pipeline agent appears in the result as soon as its file is added.
    """
...
def _read_actual_model(jsonl_path: pathlib.Path) -> Tuple[Optional[str], List[str]]:
    ...
    to parse as JSON rather than stopping at the first unparsable line."""
    notes: List[str] = []
...
def _read_agent_span(jsonl_path: pathlib.Path) -> Tuple[Optional[Tuple[float, float]], List[str]]:
    ...
    timestamp rather than stopping at the first such line. Returns
    (None, notes) - a missing span rather than a guessed one - when the
    file is absent, unreadable, or yields no parsable timestamp at all."""
    notes: List[str] = []
    if not jsonl_path.exists():
        notes.append(f"sibling {jsonl_path.name} not found")
        return None, notes
...
def build_concurrency_map(
    ...
    counting the dispatch itself - have a span (see _read_agent_span) that
    overlaps that dispatch's own span. any_span_available is False only
    when nothing under subagents_dir yielded a usable span at all, after
    which the caller reports the concurrency dimension as unavailable for
    the whole session rather than one dispatch at a time. An agent whose
    own span could not be built is absent from the returned map, although
    the other agents in the session still resolve.
    """
...
def _rung_for(
    ...
    Pipeline-fixed dispatches and Fable are outside the ladder, for
    different reasons named in dispatch-authorisation.md and in
    fable-escalation-policy.md respectively, so this function reports each
    one as outside the ladder rather than assigning it a rung.
    """
    if pipeline_fixed:
...
def _build_record(
    ...
    if requested_model_field:
        ...
        requested_tier_source = "no override recorded, so the dispatch inherits the dispatching session's model"
...
def audit_session(
    ...
    """Audit one session directory (the directory that contains subagents/)."""
...
def derive_project_slug(project_root: pathlib.Path) -> str:
    ...
    This function uses os.path.abspath rather than pathlib's resolve() on
    purpose. On Windows, resolve() reads the real filesystem and silently
    corrects the case of a path segment to whatever the disk actually
    stored, which would make the derived slug depend on machine-specific
    directory case rather than on the path string alone. abspath is purely
    lexical."""
...
def find_latest_session_dir(
    ...
    subagents/ folder under projects_dir, notes). Returns None with a note
    when projects_dir is absent or carries no such session."""
    notes: List[str] = []
...
def audit_project(
    ...
    directory, and audit that directory.

    project_root defaults to the current working directory.
    claude_projects_root defaults to ~/.claude/projects. Pass
    claude_projects_root explicitly in tests, so that nothing here ever
    reads the real home directory. session_dir bypasses slug derivation and
    session selection entirely, so pass session_dir to name a session
    directly.
    """
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "subagent dispatch in a Claude Code session. Reports rather than "
            "enforces, and never exits non-zero because a dispatch looks wrong."
        )
    )
    parser.add_argument(
        "--session-dir",
...
def main(argv: Optional[List[str]] = None) -> int:
    # Windows consoles default to cp1252, so force UTF-8 on stdout and stderr,
    # which keeps a non-ASCII agent description or path from crashing the
    # report. This is the PLAN-AF2 guard, applied here for the same reason it
    # applies across scripts/ci/.
    try:
