...
**Accepted tradeoff:** "persist on every update" means sync re-adds the deny even if a consumer deliberately removed it. Given the tool's mobile bugs and the prose-first methodology, this default is intentional (PLAN-AH2 §Context).

**Out of scope:** `plan-foundry-uninstall` does not strip the bundle-injected deny (PLAN-AH2 §Out of scope).
