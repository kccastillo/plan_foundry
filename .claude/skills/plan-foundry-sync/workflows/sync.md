...
bundle_copy.quarantine(target_claude, classification.gone_upstream)  # gone_upstream ONLY
...
- Each quarantined file's recorded sha256 is compared against its on-disk hash (taken before the move) and flagged `modified_since_install` if they differ.
...
gi_status, gi_added = gitignore_entries.ensure_gitignore_entries(target_root)
```
