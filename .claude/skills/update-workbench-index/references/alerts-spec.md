...
**Remediation:** Resolve the recurring error, acknowledge it in `audit_acknowledgements` (if accepted as-is), or dispute it in `audit_disputes`. The `[STUCK xN]` badge in the severity-surface downstream is keyed to this alert.
