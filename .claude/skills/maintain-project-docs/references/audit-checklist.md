...
- B2. Key Commands - only the most-frequent (<=3 commands inline; rest as pointer). <!-- tally-ok: a cap on how many may appear, not a count of how many do -->
...
## J. Scope coverage
- J1. `.claude/skills/_shared/*.md` helper files - verify each exists and check against sections A, C, D, E as with the other in-scope files.
