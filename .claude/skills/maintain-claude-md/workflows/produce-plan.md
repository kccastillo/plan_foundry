...
- Never propose modifications inside `MARKER_BLOCK_RANGE` — if the plugin-managed content needs updating, the recommended fix is "re-run `Skill(\"init-plan-foundry\")` to refresh from the plugin's operating-rules.md".
