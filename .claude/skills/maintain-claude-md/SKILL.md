...
## Output mode (soft dependency on plan_foundry bundle)

This skill works **with or without** the plan_foundry bundle's lifecycle machinery wired in:
...
**init-plan-foundry sentinel-marker awareness (host CLAUDE.md).** When the audited CLAUDE.md contains a `<!-- plan-foundry:init-plan-foundry:start -->` ... `<!-- plan-foundry:init-plan-foundry:end -->` block placed there by the `init-plan-foundry` skill, the audit treats the content between the markers as bundle-managed:
- **Skip sections A/C/D/E** for the content between markers (the inlined operating-rules.md is owned by the bundle, not by the consumer; auditing it for line-budget or instruction-weighting drift would falsely flag bundle content).
...
- **Never propose modifications inside the markers.** If the inlined content needs updating (e.g., the bundle's `operating-rules.md` evolved), the human re-runs init-plan-foundry to replace the between-markers content from the current bundle version.
...
When run in **audit** mode (monthly RECUR- or on-demand), the skill reads the current month's curated lessons and identifies **promotion candidates** - lessons with `recurrences: N` where `N >= 3`. These are recurring observations that the human may want to codify into CLAUDE.md as permanent operating rules.
...
The monthly RECUR- task is a visibility mechanism, not auto-execution - its own PLAN frontmatter is the cadence record; the human or the active session triggers it.
</essential_principles>
