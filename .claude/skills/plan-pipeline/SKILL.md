...
Ideation is interactive and runs in the parent session only — invoke `Skill("ideate")` directly (plugin-qualified — bare `Skill("ideate")` fails across plugin boundaries), never via a subagent. The structural safeguard is the absence of an `ideate-runner` agent file. (Decision 10.)
...
- Never run `ideate` as a subagent. Invoke `Skill("ideate", ...)` in the parent session (plugin-qualified; the ideate skill lives in the `ideate` plugin, not `plan-foundry-core`).
