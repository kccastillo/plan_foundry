...
Kanban full-stop on `outcome: exception` from any subagent OR from the iteration ceiling raised by `lib/build_brief.py` OR from a git failure. Halt the pipeline; commit + push the WIP state with diagnostics; do not advance. The Human decides recovery. (Decisions 19, 22.)
Audit loops are synchronous, durable, and bounded. Sufficiency runs first; plan-safety runs only after sufficiency returns success. Each iteration is announced to the Human (no silent re-runs). MAX_ITERATIONS=5 per stage, derived from disk and enforced in code, not counted by the orchestrator. (Decision 21.)
...
Decision-15 triage on every Human-facing surface: classify items before asking. Only Real-judgement-call items become questions. Taxonomy definitions: See `../audit-sufficiency/workflows/audit-sufficiency-steps.md` - Step 4. (Decision 15.)
...
Ideation is gated on proportionality. `ideate` runs Gate 0 first and presents the four-rung menu from [../_shared/proportionality-gate.md](../_shared/proportionality-gate.md); only `full arc` enters the drafting phase, and `audit it` enters at `drafted`. Do not start the pipeline around that gate.
...
PLAN-driven executor tier selection. Read the target PLAN's `assigned_to:` frontmatter; route to `plan-executor` (haiku, default), `plan-executor-sonnet`, or `plan-executor-opus`. `human` is the exception: **do not dispatch** - halt at `checked` and hand the walk to the Human, because such a PLAN's steps sit outside the executor capability boundary. The executor-tier table in [references/phase-state-machine.md](references/phase-state-machine.md) is authoritative for every value, including any this line does not name. `assigned_to` is a guideline (decision 8) - Human can override per PLAN. Opus is the ceiling for dispatch; Fable is never a tier the orchestrator selects. See [../_shared/fable-escalation-policy.md](../_shared/fable-escalation-policy.md) for the escalation-only rules that govern it.
...
- `build_brief.py` refuses to build a brief because the stage already has MAX_ITERATIONS=5 audits on disk ("audit loop did not converge").
