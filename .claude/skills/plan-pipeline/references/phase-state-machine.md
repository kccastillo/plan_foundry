...
| sufficiency              | revision_needed            | `plan-auto-fixer` (foreground) - when `triaged_human_items` has no `real_judgement_call` entries AND `auto_fix_iterations < 2` (mtime-gate bypassed on auto-fix path); otherwise `sufficiency-auditor` (after Human revision) | `auto_fix_iterations` (auto-fix path) or `sufficiency_iterations` (human-revision path) |
| sufficiency              | exception (from auto-fixer) | Kanban full-stop: surface auto-fixer diagnostics to human | - |
...
**Auto-fix path (PLAN-AH1 D1):** The auto-fix row takes priority over the existing "surface to human" row when: (a) `triaged_human_items` contains no `real_judgement_call` entry, AND (b) `auto_fix_iterations < 2`. In all other cases (`real_judgement_call` present, or `auto_fix_iterations >= 2`), the existing human-surface row applies unchanged.

**Auto-fix scope:** applies to `last_stage: sufficiency` only. The `plan_safety` stage does not emit `triaged_human_items` - all plan_safety `revision_needed` outcomes follow the existing human-surface path unchanged.

**mtime gate carve-out:** the re-revision mtime no-op gate ("if mtime is NOT advanced and `last_outcome: revision_needed`, orchestrator no-ops waiting on human") is bypassed on the auto-fix path. When `auto_fix_iterations < 2`, the orchestrator dispatches the auto-fixer even without a mtime advance on the PLAN file.

**Counter composition:** on auto-fixer success, re-dispatch the same auditor stage. That re-dispatch increments `sufficiency_iterations` against the existing `MAX_ITERATIONS = 5` ceiling. The composite loop is doubly bounded: max 2 auto-fix rounds (`auto_fix_iterations < 2`), each consuming at most one additional sufficiency iteration toward the 5-iteration cap. An auto-fix round that exhausts `sufficiency_iterations = 5` triggers the existing MAX_ITERATIONS kanban-stop path.

**Commit message template for auto-fix:** `plan-pipeline: auto-fix attempt <N> for <plan-filename>`.
...
| drafted (audit, auto-fix - PLAN-AH1 D1)         | `plan-auto-fixer`                                                         | foreground | `{plan_path, audit_review_text, triaged_human_items}`      | `{fixes_applied, skipped_items, executor_notes}`     |
...
Audit JSON under `Workbench/.audit/` and both INDEX projection files (`Workbench/INDEX.md` and `Workbench/.index.json`) are tracked and are staged by name per the `## Staging scope` section below; `.gitignore` is the authority and this table must agree with it.
...
audit_state:
  ...
  auto_fix_iterations: 0     # number of auto-fixer dispatches against this PLAN revision (PLAN-AH1 D1);
                             # reset to 0 alongside sufficiency_iterations / plan_safety_iterations when
                             # the PLAN reverts to drafted. Ceiling: 2 (auto_fix_iterations < 2 to dispatch).
...
```
Reset to fresh (all zeros / `none` / empty strings) when reverting from `executing` or `outcome-verifying` back to `drafted` (PLAN content has changed; prior audits are stale). `auto_fix_iterations` is reset alongside `sufficiency_iterations` and `plan_safety_iterations` on `drafted`-revert.
...
PLAN frontmatter `status` (projected by INDEX.md, regenerated on every commit-worthy phase transition) is the sole canonical surface for PLAN state. Phase-transition commits touch the PLAN file and INDEX.
...
One expected instance of this: the `outcome-verification ran` commit stages `PLAN` only, yet leaves `Workbench/INDEX.md` and `.index.json` transiently dirty, because the orchestrator's own `verify:` items run `update-workbench-index` as a side effect. This is not an omission - the following `INDEX regen` commit at 4G picks up that dirt on its own turn.
...
| `INDEX` | `Workbench/INDEX.md` and `Workbench/.index.json` | Tracked; staged by the INDEX-regen commit in `dispatch.md` Step 4G. Nothing stages these automatically. `.claude/hooks/pre-commit` used to regenerate and stage them and no longer does - its own body records why ("that silently added two files to whatever the commit was about, which is the opposite of staging what the work touched"), and its only remaining action is the CLAUDE.md line cap. The orchestrator is therefore the only stager of the INDEX pair, and it stages them by name. |
...
| `INPUT` | `Workbench/<input-filename>` | The RESEARCH or ADVICE file `write-input` just wrote. |
...
| CLAUDE.md drift check fired (complete sub-step)  | `plan-pipeline: claude-md-drift-check ran for <plan-id>`                                            | `PLAN`, plus the files `maintain-claude-md` reported writing, named at commit time |
...
| INDEX regen (after any phase-transition commit)  | `plan-pipeline: update-workbench-index`                                                             | `INDEX` |
