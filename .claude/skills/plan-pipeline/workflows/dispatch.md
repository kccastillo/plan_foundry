...
The resolver checks the PLAN's frontmatter `push_policy:` field, and when that field is absent or invalid the resolver falls back to the hard-coded default `"manual"`. (Per PLAN-AC4 D6/D6a, the earlier `.claude-plugin/marketplace.json` project-default layer was removed - the value had never been set to anything other than `"manual"` in tree.)

- `policy == "auto"` -> push as today (run `git push` after every commit).
- `policy == "manual"` -> skip push and emit `<push_status>commit landed locally; push manual per push_policy. Run \`git push\` when ready.</push_status>`.

All references to "commit + push" in this document are subject to this conditional. The commit always happens, while the push is gated on `policy == "auto"`.
...
Per-phase routing tables, commit-message templates, and the frontmatter mutation cheat sheet are recorded in [../references/phase-state-machine.md](../references/phase-state-machine.md). This file carries the procedural narrative, and that reference carries the lookup.
...
Note: `total_loop_attempts` is included in `OWNED_FIELDS` so the snapshot/restore guard protects that field from subagent writes. The field is managed by the Routine - the server-side hourly cron trigger the `autonomous-loop` skill creates, defined in [../../autonomous-loop/SKILL.md](../../autonomous-loop/SKILL.md) - rather than by the pipeline, and is not zeroed on `drafted`-revert, because unlike `audit_state` its purpose spans resets to maintain the cross-session IAL guard (IAL: infinite agentic loop, the non-termination failure mode PLAN-AH1 FM2 bounds). The orchestrator reads `total_loop_attempts` but must not reset it during audit-state resets.

Note: `status` is intentionally excluded from the guard. The executor legitimately writes `status`, and `status` falls outside the BUG-3 forgery surface. Guarding `status` would risk reverting a legitimate executor-set value.
...
**D1a carve-out ("Orchestrator-Directed-Writes").** When the orchestrator itself *directs* a subagent to write an owned field - specifically `plan-writer`'s `target_phase` parameter, which has plan-writer atomically write a new `pipeline_phase` alongside body content - the orchestrator MUST set that field in the snapshot dict to its *intended post-dispatch value* before calling `restore_owned_fields`. This preserves the orchestrator-directed change while still wiping any other owned-field write the subagent made. The guard wipes undirected subagent owned-field writes, but never reverts a value the orchestrator deliberately requested.

### Verify, don't trust the narrator

**Principle (D4 - "Verify-Dont-Trust-Narrator").** The orchestrator independently verifies any state change a subagent claims against disk and git before acting. A subagent's return text is an assertion, not evidence. The orchestrator confirms claims against the filesystem or git log before treating them as true.

The canonical prior instance of this principle is branch 4F, headed "4F. `complete` (retire + drift check)" (retire post-condition check, PLAN-AA2): when `plan-retirer` reports `outcome: success`, the orchestrator independently stats the source and destination paths before committing. The 4F pattern is the template, so all other subagent dispatches apply the same verification discipline.

**Observed instance (2026-06-18 - BUG 3 / BUG 4 incident).** During this incident, the "trust the narrator" pattern failed concretely in the following ways:
...
2. A `plan-retirer` subagent reported it had "pushed to main", although git showed nothing was pushed. Checking disk and git caught the false report, which reading the subagent's account alone would not have done.

Subagents are unreliable narrators of their own side effects. Never treat a subagent's claimed outcome as authoritative without confirming that outcome against the filesystem or git log.

The snapshot/restore mechanism guards this owned-field set: `pipeline_phase`, `audit_state`, `last_executor_outcome`, `verification_state`, `total_loop_attempts` (status excluded - see D1 above).
...
**If `lock_result["acquired"] is True`:** proceed to Step 1. The lock MUST be released (via `release(repo_root)`) on every return path from this invocation - including exception/halt paths, idempotent no-ops, and all kanban-halt branches in branch 4G-halt. The release is a finally-style guarantee: every code path that passes Step 0 acquisition calls `release(repo_root)` before returning control to the parent session. Exception and halt branches in Step 7 and branch 4G-halt MUST also call `release(repo_root)` before returning.
...
- Read the file. If unreadable -> emit `exception` (orchestrator-side, and no commit is required since nothing changed).
...
**If both supplied:** prefer `plan_path` and treat `request` as additional context only.
...
- **`pipeline_phase: drafting` AND (`ideate_phase ∉ {complete, exited_early}` OR `ideate_phase` field absent):** the PLAN is being actively shaped by the ideate cadence pipeline. plan-pipeline takes **no action (no-op)**. Return with: "PLAN is being ideated (ideate_phase: <value>); plan-pipeline waits until ideate_phase transitions to `complete`. Re-invoke after ideate completes." This makes `drafting` a long-duration phase that ideate owns. plan-pipeline must not advance or modify the PLAN until ideate hands off by flipping `ideate_phase: complete` + `pipeline_phase: drafted`.
  - Exception: if `ideate_phase` is absent and `pipeline_phase: drafting`, apply the same no-op. The PLAN may be in phases 1-3 (conversational, with no `ideate_phase` set yet). plan-pipeline waits.
...
- `pipeline_phase: executing` AND no `last_executor_outcome` recorded -> executor still running (or no completion message arrived), so read `Workbench/.heartbeat/<plan-id>.json` if that file exists and apply the following sub-branches:
...
**Children gate.** If `triggers_plans` is non-empty, read each child PLAN's `status`. If any child is non-terminal (not in {`done`, `partially-complete`, `cancelled`, `closed`}), surface "Paused for children: <list>" with decision-15 triage, then return. Do not advance the parent's `pipeline_phase`.
...
- (c) If the in-flight set is non-empty AND the target PLAN is not in that set (i.e. this is a distinct plan being started while others are active):
  ...
  - This is a **soft warn-and-proceed** - the orchestrator does not halt and does not require Human confirmation. The warning is visible, and the operator decides whether to pause manually.
...
The plan-boundary check uses no new frontmatter fields and requires no state file, because the orchestrator reads existing `pipeline_phase` values on every invocation.
...
4. Input escapes during ideate: ideate invokes `write-input` directly, and the resulting filenames are recorded in the PLAN's `linked_inputs:` on the next `plan-writer` checkpoint. Commit + push after `write-input` returns (template `plan-pipeline: drafting input <filename>`).

Return control to parent after each `plan-writer` checkpoint (the arc itself is conversational, so the orchestrator re-enters when the Human's next turn re-triggers the pipeline).

### 4B. `drafted` (audit loop)

**Patch-application procedure (PLAN-AJ0, sufficiency stage only, per D10).** On a `sufficiency | revision_needed` return, attempt this procedure before falling through to the severity-surface wiring below - a demotion out of this procedure is what reaches the severity surface.
...
2. Join each `mechanically_forced` entry in `triaged_human_items` to its finding in `diagnostics.findings` on `code` plus `location`. Per D3, if the pair matches more than one finding in the round, send every finding the pair matches straight to the human surface and apply none of their patches.
3. Call `pre_human_bound_reached(plan_path, current_iteration, bound=2)` from `.claude/skills/plan-pipeline/lib/patch_gate.py`, passing the path of the PLAN under audit and, as `current_iteration`, the iteration number of the round `apply_audit_outcome` has just written. If the call returns `True`, route the whole round to the human surface and do not attempt the patch path. If the call raises `DefectiveAuditRecord`, route the whole round to the human surface as well, as a defective-record signal, with the exception's message attached to the surfaced diagnostics, because a present-but-broken audit record must not be read as a round that never happened and the round must not be retried silently. Catch `DefectiveAuditRecord` by its type, not by catching every exception: an unrelated failure such as an `ImportError` or a missing module also produces a traceback but is not a defective-record signal, so let that failure halt the pipeline as a kanban halt rather than routing the failure as a defective record.
...
5. Call `renumber_steps` (from `.claude/skills/plan-pipeline/lib/step_renumber.py`) on the patched PLAN text. When the returned remap is non-empty, write the renumbered text back, call `remap_plan_fingerprints` (from `.claude/skills/plan-pipeline/lib/step_remap.py`) with that remap and write the updated frontmatter back, append the `renumber_report` output and the remap report to the round's audit snapshot under `orchestrator_note`, and append `{iteration: <N>, map: <remap>}` to the PLAN's `step_remaps` frontmatter list. This happens inside the same commit as the patch application in item 4.
   ...
   Either exception `renumber_steps` can raise routes the whole round to the human surface rather than being repaired - name the exception class in the surfaced message rather than treating both the same way, because they mean different things: `RefusedRenumber` fires whenever D6's predicate rejects the patched block's ordinal sequence, which is the likelier of the two on a real PLAN under audit, while `ValueError` fires only when the block has no `## Steps` heading, which cannot be true of a PLAN mid-audit-loop. On `RefusedRenumber`, surface the exception's ordinal sequence, leave the patched text in place unrenumbered, and do not call `remap_plan_fingerprints` at all, because there is no remap to carry and calling `remap_plan_fingerprints` with an empty remap would write a misleading empty entry to `step_remaps`.
...
7. Commit (template `plan-pipeline: applied <N> auditor-supplied patches for <plan-filename>`), then re-dispatch `sufficiency-auditor`.

On a mixed round of applied and demoted patches the orchestrator re-dispatches rather than waiting on the human, and does not surface a partial round and wait. The orchestrator does not retry a demoted patch and does not author a replacement for one, because a demoted finding simply recurs in the next round with its fingerprint intact. The orchestrator never applies a patch to any file other than the PLAN under audit.
...
- Reply contains `fix`: commit mutations, await PLAN mtime advance, then re-dispatch the auditor on next re-entry.
- All errors covered by `override` actions (no `fix`): treat as audit success for this stage, then advance.
- Only `ack`/`dispute` on warnings or notes, with no errors: treat as success, then advance.
- Errors remain unresolved: re-surface "errors remain - reply 'fix' or 'override <ID>: <reason>'."
- `extract` action: dispatch `Skill("write-plan")` to seed the child PLAN, then link `triggers_plans` on the parent.
- `different-auditor` action: write `audit_state.preferred_model_override`, reset the stage iteration counter, then await re-invocation.
...
**The helper is the iteration ceiling.** The helper raises `OrchestratorException` instead of writing a brief once the stage already has `MAX_ITERATIONS = 5` audits on disk, with the message `"audit loop did not converge after 5 iterations on <auditor>"`. Treat that failure as the kanban halt at branch 4G-halt - emit `outcome: exception`, commit and push WIP, surface to the Human. Do not retry the helper, and do not dispatch the auditor without a brief. This is the only enforcement of the cap, and the orchestrator counts nothing itself.

**Before surfacing that halt, dispatch the ceiling diagnostician.** See "Ceiling diagnosis" below. The cap stops the loop, but does not excuse handing the Human a bare failure.
...
The orchestrator reads `/tmp/audit_brief_<plan-id>.md` and embeds the contents verbatim as the user-message content in the `Agent()` dispatch.
...
5. The next auditor dispatch (sufficiency -> plan-safety, or revision re-run) builds its brief via `build_brief.py`, which has a precondition fingerprint check (added by PLAN-AE0): the helper refuses to dispatch if the expected commit-pair housekeeping is not present in git history. This is the belt-and-braces backstop - if the helper from step 2 was not called (orchestrator skip), the next stage fails loudly at brief-build time.

Delete `Workbench/.audit-tmp/<plan-id>-<stage>-<iteration>.json` after parsing, because that file is transient and the canonical snapshot is kept in `.audit/`.
...
| sufficiency  | revision_needed    | Awaiting Human revision. If PLAN mtime advanced since the audit_state write, re-dispatch `sufficiency-auditor`, otherwise no-op. |
| plan_safety  | success            | Both passed -> flip `pipeline_phase: checked`, commit + push, then return. (No further dispatch this invocation.) |
...
5. Branch on `result_dict["outcome"]`:
   ...
   - `revision_needed` -> frontmatter already mutated by helper. Apply decision-15 triage to `result_dict["review_text"]` if present, surface iteration counts, then instruct the Human "Revise the PLAN and re-invoke me to re-audit." If `result_dict["narrative_findings_mismatch"]` is true, surface the mismatch alongside the findings: the auditor's reported `blockers_count` disagrees with the number of error-level findings in `diagnostics.findings` for this same return, so a finding may have been named in the narrative and dropped from the structured array a revision only checks against. Return.
   - `exception` -> kanban halt. Commit + push WIP with diagnostics. Surface diagnostics. Return.

**Ceiling diagnosis (mandatory whenever the cap fires).** The cap can fire either through `build_brief.py` raising `OrchestratorException` or through the pre-dispatch counter check above, and both take this path before anything is surfaced to the Human.

1. Dispatch `audit-ceiling-diagnostician` foreground, with `{plan_path, stage, audit_dir: "Workbench/.audit"}`. The diagnostician is pinned to Opus and is read-only: it never edits the PLAN, writes to `.audit/`, commits, or advances `pipeline_phase`.
2. Write the diagnostician's `payload` into a `## Kanban halt` section in the PLAN body - the diagnosis, the recommended next step, the alternatives with their costs, and the tier-fit judgement. The orchestrator writes this section, not the diagnostician, so the No-Trust-Subagent-State principle holds.
...
The diagnosis does not resolve the halt and does not lift the cap. `build_brief.py` still refuses a sixth brief, and the recommended step is carried out by the Human or, once they choose that step, by the orchestrator in a fresh pass. Do not treat a recommendation as authorisation to continue the loop, and do not delete or move audit JSONs to reset the derived counter - the counter is derived from those files so that a caller cannot reset the counter.
...
1. Read PLAN's `assigned_to` frontmatter. Map per the executor-tier table in references/phase-state-machine.md:
   ...
   **`assigned_to: human` halts here.** Leave `pipeline_phase: checked`, dispatch no executor, and surface: "PLAN <filename> declares `assigned_to: human`. Its steps are outside the executor capability boundary, so the orchestrator will not dispatch one. Drive the steps in this session, then re-invoke at `outcome-verifying` when they are done." This is not a kanban halt - nothing has gone wrong and no WIP commit is needed. This branch is the routing outcome for a PLAN the pipeline audits and verifies but does not execute. Without this branch the value falls through to `unrecognised` and an executor is dispatched against steps it is structurally unable to perform, which is the PLAN-009 regression shape. **Return - do not continue to step 2.**
2. Flip `pipeline_phase: executing`. Set `status: in-progress`. Commit + push (`plan-pipeline: executing <plan-filename>`).
3. Dispatch the executor with `run_in_background: true`. The Agent call returns an agent ID, and the orchestrator does not wait. Surface to the Human: "Executor <name> dispatched in background. I'll resume when it completes."
4. Return control to parent. The executor's completion message arriving in parent's conversation is the re-entry cue.

### 4D. `executing`

**Re-entry path.** Parent-Claude observed the executor's completion message and re-invoked this skill with `plan_path`. The orchestrator does not poll, so this branch only runs on re-entry.
...
1. **Executor-return validation** (Reeve hiccup H10 / 2026-05-16). Before reading `last_executor_outcome` or treating any completion message as authoritative, validate the executor's return contains a well-formed `<pipeline-result>` block. Specifically:
   ...
   - If the result is empty, contains a rate-limit string (e.g. `"You've hit your limit"`, `"resets at"`, `"rate-limited"`), or otherwise lacks the expected block: do not trust `last_executor_outcome` (the field may not have been written, or may be stale from a prior run). Treat as `outcome: exception` with `diagnostics.reason = "executor return malformed: <empty | rate-limit | missing pipeline-result block>"`. Preserve the heartbeat and take the kanban halt. Commit + push WIP. Surface to Human with the raw completion text for inspection. Return.
   - This is structurally identical to the retire-skill post-condition pattern (section 4F step 3): the harness's "completed" status conflates "executor finished successfully" with "executor's invocation returned a result of any kind", so the orchestrator MUST verify the structured block exists before progressing.
2. Read `last_executor_outcome.outcome` from PLAN frontmatter (the executor's `execute-plan` workflow writes this - decision 24).
3. If absent or stale (executed date older than the dispatch) -> no-op. The completion message likely fired before the executor finished, so wait for the next message.
4. Parse the most recent `<pipeline-result>` block from the executor's return (parent-Claude supplies the block implicitly via the conversation. If the block is not directly available, treat the frontmatter `last_executor_outcome` as canonical, which is the durable record per decision 24).
5. Branch:
   - `outcome: success` -> **Delete `Workbench/.heartbeat/<plan-id>.json`** immediately (the heartbeat is no longer load-bearing and would otherwise mislead `/status` and INDEX alerts). Flip `pipeline_phase: outcome-verifying`. Commit + push (`plan-pipeline: outcome-verifying <plan-filename>`). Continue into branch 4E within the same invocation.
   - `outcome: revision_needed` -> **Delete `Workbench/.heartbeat/<plan-id>.json`** immediately (the executor is done, so the heartbeat is stale). Revert `pipeline_phase: drafted`. Reset `audit_state` to fresh (`last_stage: none`, `last_outcome: none`, counters to 0) - content has changed materially, so prior audits are stale. Set `status: needs-revision`. Commit + push (`plan-pipeline: executor revision_needed <plan-filename>`). Surface executor's `diagnostics_summary` + Executor Notes pointer with decision-15 triage. Return.
   - `outcome: exception` -> **Preserve `Workbench/.heartbeat/<plan-id>.json`** (forensic data attached to the halt, and deleted on the next phase transition out of halted state - see heartbeat lifecycle table below). Take the kanban halt. Commit + push WIP (`WIP: pipeline halted at executing for <plan-filename> - see diagnostics`). Surface diagnostics. Return.
...
**Severity-surface wiring (outcome-verifying, branch 4E).** After running shell checks, the orchestrator invokes the outcome-verifying surface modules:
...
4. For each `verify: orchestrator` item: the orchestrator performs the verification itself (or via a dispatched non-executor verifier). Record a line-by-line attestation into `verification_state.orchestrator_attestations`:
   ...
   ```
   A `verify: orchestrator` item does not add to `human_pending`. A `verify: orchestrator` pass contributes to the "all checks passed" branch condition, while a fail routes like a shell failure (auto-revert unless a human item co-occurs).
...
7. Branch:
   - **Any `verify:`/`acceptance:`/`verify: orchestrator` check failed AND `human_pending` is empty** -> pure shell failure, so auto-revert (existing behaviour). Revert `pipeline_phase: drafted`, reset `audit_state` to fresh, set `status: needs-revision`. Commit + push (`plan-pipeline: outcome-verification failed - reverting to drafted for <plan-filename>`). Surface failed-assertion list with decision-15 triage. Return.
   - **Any `verify:`/`acceptance:`/`verify: orchestrator` check failed AND `human_pending` is non-empty** -> mixed failure, so invoke `render_outcome_surface()` with both the shell failures and the human items. Human may `ack-failure` non-blocking shell failures before issuing `pass`. Route per severity-surface state-machine rows in phase-state-machine.md.
   ...
   - **All shell checks passed AND `human_pending` non-empty** -> invoke `render_outcome_surface()`. Surface the structured severity-surface prompt. Return. The Human's reply re-triggers the orchestrator. On re-entry while `pipeline_phase: outcome-verifying` AND `human_verdict: pending`:
     - Parse the reply via `parse_outcome_reply()`, then apply the parsed action via `apply_outcome_action()`.
     - `pass` (all failures acked, all human items acked) -> flip `pipeline_phase: complete`, then commit + push.
     - `fail` or `reject-human` -> revert `pipeline_phase: drafted`, reset `audit_state`, then commit + push.
...
When building each `human_items` entry passed to `render_outcome_surface()`, the orchestrator populates the following optional evidence fields in addition to the existing `id` and `prose`:
...
For `verify: human` items with attestation-then-assent, the orchestrator sources the `evidence` field from `orchestrator_attestations` and inlines that field. The human then replies `accept-attestation H1`, `veto H1: <reason>`, or `dig-into H1` rather than performing the check themselves:
- `accept-attestation H1` -> records that the attestation was accepted, contributes to overall `all_pass`, and flips -> `complete` once all items are resolved.
- `veto H1: <reason>` -> treats the item as `rejected`, reverts -> `drafted`, and resets `audit_state`.
- `dig-into H1` -> the orchestrator surfaces the full evidence set for that item only, then the human re-prompts with `accept-attestation` or `veto`.
...
1. Run the failing command and capture the full output.
...
3. **Post-condition verification** (PLAN-AA2 defence-in-depth pattern). When the subagent reports `outcome: success`, the orchestrator MUST independently verify the retire skill's success_criteria against the filesystem before committing. This catches the 2026-05-13 class of bug where the subagent self-reported success but `git rm`'d the file instead of moving the file. Specifically, before treating `success` as success:
   - Compute expected destination: `<repo-root>/Retired/<original-basename>` (computed via `git rev-parse --show-toplevel`, with the basename derived from the PLAN path passed to the subagent).
   - Stat the source path - assert the source path does not exist on disk.
   - Stat the destination path - assert the destination path exists, is a file, and has non-zero size.
   - If any assertion fails: override the subagent's reported outcome - treat that outcome as `outcome: exception` with `diagnostics.reason = "post-condition violation: <specific failing check>"`. Halt and do not commit. Surface to Human via kanban-halt (branch 5) including the subagent's claimed-success message for contrast.
...
5. **Compact suggestion (PLAN-AB5, H7).** After the retire commit (step 4 success), emit the following structured tag to the human:
   ...
   Replace `<plan-filename>` with the actual filename of the PLAN that was just retired. The Claude Code harness picks up the `<compact_suggestion>` tag as a cue to run `/compact`, and in a harness that does not support the tag the line is harmless prose. This surface exists to prevent the boundary-discipline failure pattern from H7 (PLAN-AB5): the orchestrator eagerly starting the next plan's ideate before compacting, losing working-memory continuity.

6. **Doc-drift check** (after successful retire, auxiliary). Read the just-completed PLAN's `files_touched` frontmatter. If any path matches any of the following patterns, foreground-dispatch `Skill("maintain-project-docs")` with no arguments - that skill audits the live state of CLAUDE.md and ARCHITECTURE.md against the current codebase (AGENT_RULES.md was dissolved 2026-05-14 per Option Y of ADVICE-003, and its meta-policy is now stated in CLAUDE.md):
   - `.claude/skills/*` (any skill change)
   - `.claude/commands/*` (any slash command change)
   - `.claude/skills/_shared/*` (invariants and plan-safe are defined here - ARCHITECTURE.md driver)
   - `.claude/agents/*` (executor capability boundaries - ARCHITECTURE.md driver)
   - `CLAUDE.md` itself
   - `ARCHITECTURE.md` itself

   Surface the skill's output to the human as a one-line summary: "Drift check: <outcome> - <brief description>". If the audit reports drift, the human invokes the produced PLAN at their discretion. If `files_touched` is absent or empty, skip silently. Failure of `maintain-project-docs` is logged (one line) but does not halt or revert the pipeline - this step is auxiliary.
...
7. **Auto-retire absorbed inputs (PLAN-AC0) + disposition cascade (PLAN-AF8).** After the retire commit (step 4 success) and drift check (step 6 auxiliary), walk the just-completed PLAN's `linked_inputs` frontmatter array. For each input file path:
   ...
   **Phase A - existing auto-retire path (three-gate):**
   ...
   - **Gate 2 - integration_status:** if `integration_status` is not already `integrated`, set it to `integrated` on the input's frontmatter and continue to Gate 3. The retire is the confirmation: a PLAN that reached `complete` with this input in its `linked_inputs` consumed that input. Surface one-line: "Input <path> marked integrated by PLAN <plan-basename> retire."
   ...
   - **Robustness:** if any input read fails (file missing, malformed frontmatter), log one line ("input <path> unreadable - skipping auto-retire") and continue with remaining inputs. Do not halt the pipeline, because this sub-step is auxiliary.

   Reference-mode inputs are deliberately not auto-retired by Phase A - reference-mode is durable knowledge. Gate 3 is what protects an input that feeds several PLANs incrementally: the input stays in `Workbench/` while any consuming PLAN is still active, whatever its integration status. False-positive auto-retires would destroy in-flight context, and Gate 3 is the gate that prevents them.
   ...
   **Phase B - disposition cascade [Human] (PLAN-AF8).** Runs for each input that was not disposed by Phase A (i.e. any input the auto-retire gates skipped). This phase is `[Human]`-surfaced - the executor MUST NOT auto-decide dispositions.
   ...
   Phase B is auxiliary: if Phase B fails for any input, log one line and continue to the next input. Never halt the complete-phase pipeline for a Phase B failure.
...
3. Once all children are terminal, re-invokes the orchestrator on the parent - Step 3's children gate now passes, and 4C dispatches the parent's executor.

The orchestrator does not poll children, because re-entry is the cue.

---

## Step 6: Bootstrap exception

While the parent PLAN `202605011400_PLAN_build-plan-pipeline-orchestrator.md` is itself executing (its own `pipeline_phase` is non-`complete`), the orchestrator does not manage git for that PLAN - the Human commits manually after each `execute-plan` call, per the bootstrap instructions in that PLAN's Constraints section. This branch is detected by inspecting whether the active PLAN is the parent bootstrap file.
...
When `push_policy == "auto"` and the orchestrator is about to run `git push`, the orchestrator MUST first call the divergence guard:
...
**If `result["safe"] is False`:** do not run `git push`. The milestone commit has already landed locally, so local history is not lost. Raise the orchestrator's own `outcome: exception` (kanban halt per SKILL.md `exception_conditions`). Surface the following diagnostics to the Human:
