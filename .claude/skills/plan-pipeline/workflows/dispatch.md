...
1. Invoke `Skill("ideate", request_or_existing_path)` directly in the parent session. **Never via a subagent.** (Plugin-qualified — the ideate skill lives in the `ideate` plugin; bare `Skill("ideate")` fails across plugin boundaries.)
