...
3. Call `pre_human_bound_reached(plan_path, current_iteration, bound=2)` from `.claude/skills/plan-pipeline/lib/patch_gate.py`, passing the path of the PLAN under audit and, as `current_iteration`, the iteration number of the round `apply_audit_outcome` has just written. If it returns `True`, route the whole round to the human surface - do not attempt the patch path.
