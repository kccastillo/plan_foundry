...
The plan-pipeline orchestrator's §4D `executing` re-entry step must validate
...
Workbench/202605160300_RESEARCH_hiccup-log.md §H10.
"""
...
def test_dispatch_4D_has_executor_return_validation():
    """§4D must mention executor-return validation as a pre-step."""
    text = _read(DISPATCH)
    assert "### 4D. `executing`" in text, "§4D section missing"
    # Section content from §4D to next "### "
    ...
    assert "executor-return validation" in lower or "executor return validation" in lower, (
        "§4D is missing executor-return validation step"
    )


def test_dispatch_4D_mentions_pipeline_result_block_check():
    """§4D validation must specifically check for the <pipeline-result> block."""
    text = _read(DISPATCH)
    section = text.split("### 4D. `executing`", 1)[1].split("\n### ", 1)[0]
    assert "<pipeline-result>" in section, (
        "§4D must reference the <pipeline-result> block as the validation target"
    )


def test_dispatch_4D_mentions_rate_limit_case():
    """§4D must explicitly cover the rate-limit / empty / malformed case."""
    text = _read(DISPATCH)
    section = text.split("### 4D. `executing`", 1)[1].split("\n### ", 1)[0]
    lower = section.lower()
    ...
    assert any(kw in lower for kw in keywords), (
        "§4D must enumerate at least one specific malformed-return case (rate-limit, empty, malformed)"
    )


def test_dispatch_4D_treats_malformed_as_exception():
    """§4D must route a malformed return to outcome:exception, not progress to outcome-verifying."""
    text = _read(DISPATCH)
    section = text.split("### 4D. `executing`", 1)[1].split("\n### ", 1)[0]
    lower = section.lower()
    ...
    assert "outcome: exception" in lower or "outcome:exception" in lower, (
        "§4D malformed-return path must produce outcome:exception"
    )
    assert "kanban halt" in lower or "halt" in lower, (
        "§4D malformed-return path must halt the pipeline"
    )
