...
required language for:
  - section 4A plan-boundary-warning surface (soft warn-and-proceed when other PLANs are in flight)
  - section 4F compact_suggestion surface (emitted after successful retire)
...
# ---------------------------------------------------------------------------
# Test 1 - section 4A contains plan-boundary-check language
# ---------------------------------------------------------------------------

def test_dispatch_4A_has_plan_boundary_check():
    """plan-pipeline dispatch section 4A must contain plan-boundary-check language."""
    text = _read(DISPATCH)
    # Locate section 4A
    assert "### 4A." in text, "section 4A section missing from dispatch.md"
    lower = text.lower()
    ...
    assert "plan-boundary" in lower or "plan_boundary" in lower or "in-flight" in lower or "in flight" in lower, (
        "section 4A is missing plan-boundary check language"
    )
    ...
    assert "glob" in lower or "workbench/plan-" in lower or "pipeline_phase" in text, (
        "section 4A plan-boundary check must reference scanning Workbench PLAN files for pipeline_phase"
    )


# ---------------------------------------------------------------------------
# Test 2 - section 4F contains compact_suggestion language
# ---------------------------------------------------------------------------

def test_dispatch_4F_has_compact_suggestion():
    """plan-pipeline dispatch section 4F must contain compact_suggestion surface language."""
    text = _read(DISPATCH)
    # Locate section 4F
    assert "### 4F." in text or "4F. `complete`" in text, "section 4F section missing from dispatch.md"
    # Must reference compact_suggestion or /compact
    assert "compact_suggestion" in text or "/compact" in text, (
        "section 4F is missing compact_suggestion surface language"
    )
...
# ---------------------------------------------------------------------------
# Test 4 - section 4A plan-boundary is soft warn-and-proceed, not hard halt
# ---------------------------------------------------------------------------

def test_dispatch_4A_boundary_is_soft_warn_not_halt():
    """
    section 4A plan-boundary check must be described as soft warn-and-proceed, NOT a
    ...
    assert any(phrase in lower for phrase in (
        ...
        "section 4A plan-boundary check is not described as soft warn-and-proceed"
    )
