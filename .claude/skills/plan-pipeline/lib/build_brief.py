...
def _finding_fingerprint(finding: dict) -> str:
    ...
    audit_loop._compute_fingerprint and auditor-schema-v3.md (fingerprint
...
def build_audit_brief(
    ...
    # Top of each auditor's primary category range, per the "Namespace:" line at
    # the head of each register: audit-sufficiency/references/auditor-codes.md
    # says S001-S799 (S701-S799 is the audit-trail-durability family Lens 9
    # emits), and audit-haiku-safe/references/auditor-codes.md says H001-H499.
    code_range_top = "799" if auditor == "sufficiency" else "499"
    # PLAN-AG2/PLAN-AK1: plan-safety's mechanical checks (audit-haiku-safe Steps
    # 4a-4e) emit auxiliary code families the widened auditor-schema-v3 pattern
    ...
    aux = (
        " Plan-safety may also emit auxiliary families SFV###, PPV###, PSZ###, EBV###, and FAL###(-a..-f)."
...
You are the **{auditor_label}**. Review the PLAN at the path provided and return a structured audit result using the v3 schema (codes `{code_namespace}001`-`{code_namespace}{code_range_top}`, `{code_namespace}999` for OTHER).{aux} See your SKILL.md for the full procedure.
...
- Apply every {"lens in the sufficiency procedure" if auditor == "sufficiency" else "plan-safety criterion"} in full.
...
Apply {"every lens in the sufficiency procedure" if auditor == "sufficiency" else "every plan-safety criterion"} but focus primarily on the diff.
...
- All findings use codes from `{code_namespace}001`-`{code_namespace}{code_range_top}` / `{code_namespace}999`.{aux}
- Return a `<pipeline-result>` JSON block per your SKILL.md output schema.
"""
        return brief
