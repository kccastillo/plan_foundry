...
def _check_audit_loop_precondition(
    ...
    # Scan the entries BEFORE the update commit (more recent in time - lower index)
    # for the paired record_last_audit_commit commit within 2 positions.
    pair_found = False
    for i in range(0, update_idx):
        _, subject = commits[i]
        if record_pattern in subject:
            pair_found = True
            break
        if update_idx - i > 2:
            # More than 2 commits between the record commit and the update commit
            break
...
def main():
    ...
    try:
        ...
        audit_state = _load_audit_state_from_plan(plan_path) if iteration > 1 else None
        brief = build_audit_brief(plan_path, auditor, iteration, audit_state=audit_state)
        print(brief)
