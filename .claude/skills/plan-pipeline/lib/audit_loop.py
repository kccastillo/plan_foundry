...
Return dict schema:
    {
        ...
        "blockers_count": int | None,          # payload.blockers_count, the auditor's
                                                # own narrative-derived tally
        "narrative_findings_mismatch": bool,    # True when blockers_count disagrees
                                                # with the error-level count in
                                                # diagnostics.findings - the auditor's
                                                # narrative and its own structured
                                                # array have drifted apart
    }

Narrative-vs-structured reconciliation:
    An auditor emits two things from the same pass: `payload.review_text` (the
    narrative, with a `Blockers: N` tally) and `diagnostics.findings` (the
    structured array the orchestrator fingerprints and a remediation stage
    works from). Nothing upstream checks these agree. A finding written up in
    the narrative and dropped from the structured array survives invisibly,
    because a remediation stage is correctly built to trust the array rather
    than re-read the whole narrative for itself. `narrative_findings_mismatch`
    catches the cheapest version of that drift: the auditor's own reported
    `blockers_count` disagreeing with the number of `error`-level findings it
    actually emitted in the same return.
...
def apply_audit_outcome(
    ...
    blockers_count = payload.get("blockers_count")
    findings = audit_data.get("diagnostics", {}).get("findings", [])

    # Narrative-vs-structured reconciliation: the auditor's own reported blocker
    # tally, computed from the same pass that produced the findings array, must
    # agree with the number of error-level findings actually present. A mismatch
    # means the narrative named a finding the structured array does not carry
    # (or vice versa) - the drift a remediation stage, which trusts the array,
    # would never notice on its own. Checked against the raw findings, before
    # acknowledgement-stripping, since the reconciliation is about the auditor's
    # own internal consistency, not what survives to the PLAN.
    error_findings_count = sum(1 for f in findings if f.get("level") == "error")
    narrative_findings_mismatch = (
        blockers_count is not None and blockers_count != error_findings_count
    )
    ...
    # diagnostics.findings is mandatory on every auditor return per
    # auditor-schema-v3.md and audit-sufficiency-steps.md, which states it is
    # emitted "on every return, including outcome: success (where it is an
    # empty array)". A return missing it is malformed, not a quiet success or
    # a quiet revision, so it is routed through the pipeline's existing
    # exception surface here rather than left to degrade recurrence detection
    # in silence. This does not invent a new state - dispatch.md's routing
    # table already halts the pipeline on outcome: exception.
    if findings_absent:
        outcome = "exception"
    ...
    return {
        ...
        "narrative_findings_mismatch": narrative_findings_mismatch,
    }
