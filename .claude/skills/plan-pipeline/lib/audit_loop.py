"""
audit_loop.py — Single helper for audit-loop housekeeping in plan-pipeline §4B.
...
Bundles five previously-manual steps into one atomic call:
  ...
  3. Compute recurrence fingerprints (basis for [STUCK ×N] badge)
...
Usage (CLI — invoked by dispatch.md orchestrator via subprocess):
    ...
    # Outputs JSON result dict to stdout.
...
def _extract_plan_id(plan_path: Path) -> str:
...
def apply_audit_outcome(
    ...
    plan_id = _extract_plan_id(plan_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("iteration", type=int, help="Current audit iteration (1-based)")
