...
def test_findings_absent_forces_exception_outcome(tmp_path):
    """
    A missing diagnostics.findings array is a malformed return per
    auditor-schema-v3.md and audit-sufficiency-steps.md ("mandatory ... on
    every return, including outcome: success"). The auditor's own claimed
    outcome must not survive that malformation - the returned outcome, and
    what gets written to audit_state and the commit message, must all read
    exception even though the auditor itself claimed success.
    """
    repo_root, plan_path = _make_git_repo(tmp_path)
    payload = json.dumps({
        "outcome": "success",
        "payload": {"blockers_count": 0, "review_text": "clean"},
        "diagnostics": {"summary": {"error_count": 0, "warning_count": 0, "note_count": 0}},
    })
    result = apply_audit_outcome(
        plan_path=str(plan_path), stage="sufficiency", audit_return_json=payload,
    )
    ...
    text = plan_path.read_text(encoding="utf-8")
    fm = yaml.safe_load(text.split("---", 2)[1])
    assert fm["audit_state"]["last_outcome"] == "exception"
