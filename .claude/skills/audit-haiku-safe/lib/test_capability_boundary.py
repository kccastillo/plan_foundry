...
def test_unlisted_skill_outside_the_four_names_is_flagged(tmp_path, monkeypatch):
    ...
    plan-safe.md historically recorded, so a hard-coded name list would have
    passed both. The derived rule (D8) catches this because neither name is
    in plan-executor's preloaded skills, independent of any maintained list.
    """
    monkeypatch.chdir(tmp_path)
...
# shapes are taken from the sweep recorded in
# Workbench/.mine/C4-ebv-false-positive-classification.md, not invented: the
