...
Design rule (D8, PLAN-AK1): see `_shared/executor-capability-boundary.md` for
what an executor Step may not do and why it is derived from the dispatched
agent file's own `skills:`/`disallowedTools:` declarations, not a maintained
list of skill names.
...
_RAW_SHELL_RE = re.compile(
    r'\bBash\s*\(|\b(?:bash|sh)\s+-c\b'
    # The optional quote character covers the backtick a Step writes around a
    # command. Without it, ``Run `bash scripts/ci/run-all.sh` `` matched neither
    # this alternate nor the -c one, which is how PLAN-AL3 Step 6 reached a
    # Bash-denied executor with this lint clean.
    #
    # A bare `bash <script>` alternate, matching the invocation with no verb in
    # front of it, was tried and removed: it caught the same Steps this one does
    # and additionally fired on PLAN-AK3, where the identical command appears as
    # a description of what the repository means by green rather than as an
    # instruction. Requiring the governing verb is what separates the two, and
    # every genuine case in the corpus carries one.
    r'|\brun\s+(?:a\s+)?[`\'"]?(?:bash|shell)\b',
    re.IGNORECASE,
)
