...
Closed enum for plan-safety findings. Namespace: `H001`-`H499` (by category), `H999` (OTHER). The plan-safety auditor also emits auxiliary code families (`SFV`, `PPV`, `PSZ`, `FAL`, `EBV`) from its Step 4a-4e mechanical checks - these are documented separately in the "Auxiliary code families" section below rather than folded into the `H###` category map.
...
| H302 | excluded-operation | Step asks the executor to perform an operation outside the dispatched agent's declared capability (e.g. `retire`, `write-input`, `plan-pipeline`, `ideate`, raw Bash - the recorded instances, not the whole set) - the model-read counterpart of the derived rule Step 4e checks mechanically |
...
## Auxiliary code families (SFV / PPV / PSZ / FAL / EBV)

These families are emitted by the plan-safety auditor's mechanical checks (Steps 4a-4e in [`../workflows/audit-haiku-safe-steps.md`](../workflows/audit-haiku-safe-steps.md)), separately from the `H###` category map above. They are admitted by the widened `findings[].code` pattern in [`../../_shared/auditor-schema-v3.md`](../../_shared/auditor-schema-v3.md).
...
| EBV001 | error | capability-boundary | Step 4e - a Step in executor scope names a skill absent from the dispatched executor agent's `skills:` list, or raw `bash`/`sh` where the agent denies Bash, with no non-executor attribution in the same sentence, and `EBV001` is not in `audit_acknowledgements` |
| EBV002 | warning | capability-boundary | Step 4e - the executor agent file for the PLAN's `assigned_to` could not be read, so the boundary check did not run |
...
### EBV001 / EBV002 - capability-boundary

Use when a Step in executor scope names an operation absent from the dispatched executor agent's `skills:` declaration (or denied via `disallowedTools:`), derived from the agent file itself rather than a maintained list of names (D8, PLAN-AK1). `EBV002` is the check's own could-not-run signal, not a safety finding.
...
```json
{
  ...
  "location": "Step 20",
  "message": "capability-boundary-violation: Step 20 asks the executor to invoke 'retire', which is not in plan-executor's preloaded skills (execute-plan). Skill() from a subagent fails as a silent no-op, so route this to the orchestrator (parent session).",
