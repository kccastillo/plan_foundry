...
description: "Read a Workbench ADVICE or RESEARCH input file (or a reusable asset file under the AD6 schema) and surface its content. Two modes detected automatically from frontmatter: (1) Input mode -- flip integration_status: pending -> integrated after operator confirms absorption (folded into a PLAN's Steps/Context/shipped behaviour). Late auto-retire fires when an input is integrated after all consuming PLANs are already retired and lifecycle_mode != reference. Tolerant of legacy integration_status: open (treated as pending). Idempotent on already-integrated inputs. (2) Asset mode (AC2c) -- surface the helper or reference, prompt operator to mark asset consulted by a specific consuming_plan, then stamp last_consulted (today's ISO date) and append consuming_plan to consulted_by (cap last-20, FIFO eviction). Writes a per-asset memory pointer file to the Claude auto-memory directory so future sessions recall consulted assets. Trigger phrases: rehydrate input <path>, consume input <path>, absorb input <path> into <plan>, mark input integrated, consume asset <path>, rehydrate asset <path>, mark asset consulted."
---
