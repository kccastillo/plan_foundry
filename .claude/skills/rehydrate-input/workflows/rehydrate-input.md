...
`rehydrate-input` does NOT run `git commit` in asset mode. The commit is the calling session's responsibility (plan-pipeline section 4F commit phase if dispatched from an active PLAN execution; the operator if invoked standalone). The commit message when the caller commits is:
...
Three gates (mirrors plan-pipeline section 4F step 7 - keep aligned):
...
If Gate 3 surfaces SKIP due to active consuming PLAN(s), the input will be auto-retired later by plan-pipeline section 4F when those PLANs retire - no operator action needed.
