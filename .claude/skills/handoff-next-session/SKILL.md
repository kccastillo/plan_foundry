...
A third computed, never-hand-edited section - `## Carried-claims baseline` - gives a claim in `## Constraints & do-nots` or `## Blocking decisions` a persistent `CLAIM-<id>`, re-checks it when it names a command, catches a silent drop, and forces an unresolved repeat into `## Blocking decisions` at the escalation threshold. Definition in [references/claim-carry-gate.md](references/claim-carry-gate.md); produced at write-handoff.md Step 2.6, `{}` when no claim is carried.
