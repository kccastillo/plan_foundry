...
section. This gate is one of the sections **exempt** from the handoff's "delete empty
sections" rule, and the exempt set is listed in full at
[../workflows/write-handoff.md](../workflows/write-handoff.md) Step 3:
the gate is always populated - either a per-PLAN table, or the literal line
...
declare blockers resolved, and the loop records that verdict on disk in the PLAN's
...
- **NOT-READY** - any other phase (`drafting`, `drafted`, `executing`,
  `outcome-verifying`, `complete`). State the phase and what the PLAN awaits.

This is enforced structurally - plan-pipeline dispatches an executor only on the
`checked -> executing` transition - but the handoff must make the verdict **visible
...
show a passing re-audit for that stage, flag the claim:

> [WARNING] self-assessed, not audit-confirmed - needs sufficiency-N + plan-safety pass before `checked`.
...
- **live ideation** - an open `ideate` arc (`ideate_phase` present and not
  `complete`), conversational, possibly awaiting inputs. Name the
  phase and any pending inputs.
- **stale-in-drafting** - the PLAN remains at `drafting` but never completed an
  ideate arc, and its content shipped ad-hoc. A supersession/retire candidate,
  not live work.
...
reconciliation, flag the overlap as a **blocking decision** for the next session:

> [WARNING] scope collision: PLAN-X Track N overlaps PLAN-Y/PLAN-Z; reduce PLAN-X to its unique parts and delegate the overlap, or record the supersession. Running both as-is collides.
...
(incompletely sized -> the next session sizes the PLAN rather than treating the
gap as a blocker).
...
| AA0  | drafting | NOT-READY | M (sonnet) | live ideation - Survey; pending inputs | - |
