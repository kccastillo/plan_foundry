...
identity, these failures are invisible - a stale claim reads exactly like a
...
the claim a persistent id makes each of them computable: re-run the checkable
ones, diff the id set to catch a silent drop, and count repeats to force an
escalation.

## The `CLAIM-<id>` grammar

The id is `CLAIM-<kebab-nickname>`, assigned once by whichever agent first
writes the claim into `## Constraints & do-nots` or `## Blocking decisions`,
and reused verbatim by any later handoff restating the same claim. No sequential counter
...
re-reads the claim, same as before this gate existed.
...
The baseline is a computed, never-hand-edited fenced-yaml block, sibling to
`## Plan-state baseline`: `{claim_id: {nickname, check, carried_count}}`.
`carried_count` increments by one each time the same id is carried forward
unresolved, and the id drops out of the baseline entirely the moment the
claim is resolved or removed. When a claim's `carried_count` reaches **3**, `write-handoff.md`
...
in `claim_carry.py`, reasoned from the evidence in the report this gate closes -
`Retired/FOUNDRYREQ-horse-chestnut-brickhouse-20260805-1701-handoff-claims-have-no-persistent-identity-so-staleness-drops-and-repeats-go-unchecked.md`,
the sole `linked_inputs` entry of `Retired/PLAN-AL1_handoff-claim-carry-identity.md`.
That evidence is a scope-collision
...
kept asking for. Three restatements is already past the point that report's
evidence shows the human consequence being visible.
...
For a checkable claim, `write-handoff.md` Step 2.6a runs the claim's `check:`
...
successor would carry the claim prefixed "stale - claim no longer reproduces,
verify before trusting" instead of restating the claim as fact.
`CLAIM-scope-collision-aa2-aa3` is freeform (no `check:`) and has now reached
the escalation threshold - `write-handoff.md` requires that claim to appear under
