...
Single source of truth for datetime-stamped artefact filenames, shared by `handoff-next-session` (write side), `rehydrate-handoff` (read side), and `raise-foundry-request` (write side). PLAN-AE8 introduced the convention for handoff files, PLAN-AF7 extended the grammar to add the datetime + gist-slug convention (per PLAN-AF6 D1), and PLAN-AH0 broadened the scope to cover FOUNDRYREQ artefacts and the shared validator contract (D6).
...
- **Live (new, mandatory for all new writes, per PLAN-AF6 D1 + PLAN-AH0 D1):** `Workbench/HANDOFF-YYYYMMDD-hhmm-<slug>.md`
  ...
  - `<slug>` is a lowercase-kebab gist summary of the handoff's headline content, authored at write time by the write-time agent, distinct from the thread scope. The slug is a **discovery aid** - it summarises the handoff's headline content so a reader listing `Workbench/` can triage without opening the file - and is **never a substitute** for the exploded handoff body, because the full context (motivation + reasoning) is recorded in the body per the handoff content contract (per ADVICE-018).
...
- **Live grammar (per PLAN-AH0 D2, D3):** `Workbench/FOUNDRYREQ-<origin>-YYYYMMDD-hhmm-<slug>.md`
  - `<origin>` identifies the originating repo or workspace. The write-time agent derives `<origin>` from the `git remote get-url origin` basename (e.g. `my-project` from `https://github.com/acme/my-project.git`), falling back to the working-directory name when no remote is configured (D3).
...
Both HANDOFF grammars are valid and discoverable during coexistence, and existing files are never bulk-renamed:

- **Old grammar:** `HANDOFF-<scope>.md` (e.g. `HANDOFF-dungeon-jaquays.md`) - remains valid, and old files keep their names.
- **Reserved default (legacy read/retire-only):** `HANDOFF-NEXT-SESSION.md` - still discovered and retired using the `NEXT-SESSION` scope token, and never written as a new handoff from PLAN-AH0 D1 onward.
...
The discovery glob `Workbench/HANDOFF-*.md` intentionally spans all HANDOFF forms. Write-time agents author the new grammar for new handoffs, while existing handoffs are read and retired unchanged.

## Discovery

- `rehydrate-handoff` discovers handoffs with the glob `Workbench/HANDOFF-*.md`. Case-insensitive matching is tolerated on read (older hand-written files may be mixed-case), and the write side normalises the slug to lowercase-kebab. This glob intentionally spans old `HANDOFF-<scope>.md`, legacy `HANDOFF-NEXT-SESSION.md`, and new `HANDOFF-YYYYMMDD-hhmm-<slug>.md` filenames.
- Reads use `encoding='utf-8', errors='replace'` semantics.

## Retire destination

- **HANDOFF:** `Retired/HANDOFF-<scope>-{YYYYMMDDHHMI}.md`, where `<scope>` is derived from the source file (see Retire scope derivation below). The timestamp suffix avoids collisions with prior retirements, including multiple same-day retirements.
...
- **New datetime grammar (`HANDOFF-YYYYMMDD-hhmm-<slug>.md`):** the filename carries a gist slug, not a thread scope. The scope for the retire destination is recovered from the handoff body's scope banner (per write-handoff.md Step 4). If no banner is present, fall back to the gist slug. This fallback fires only for a malformed new-grammar handoff with no banner, because in normal operation scoped handoffs always write a banner.

## Per-scope retire rule (the core invariant)

Writing or retiring scope X touches **only** the relevant handoff file for that scope. No invocation ever reads, moves, or overwrites a *different* scope's handoff. This per-scope (never global) rule is what allows multiple thread handoffs to coexist, and that coexistence is the entire point of the multi-thread expansion.

## Validator contract (D5 - shared, per PLAN-AH0)

The shared validator at `.claude/skills/_shared/validate_artefact_filename.py` classifies every HANDOFF, FOUNDRYREQ, and OBSERVATION basename into one of the classes below, or treats the name as not-subject:
