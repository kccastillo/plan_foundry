...
{ANY NON-OBVIOUS CONVENTIONS THE NEXT SESSION SHOULD INHERIT — naming patterns, harness rules from CLAUDE.md, gotchas about bundle paths, AU spelling, etc. Lean on what's NEW since the last handoff; assume the next session has read CLAUDE.md and ARCHITECTURE.md.}
...
2. Run quick state-check commands ({EXAMPLE: `git log --oneline -10`, `ls Workbench/`, `python .claude/skills/write-plan/scripts/next_id.py PLAN`}).
