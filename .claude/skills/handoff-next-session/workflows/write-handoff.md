...
Full definition: [../references/claim-carry-gate.md](../references/claim-carry-gate.md).
Uses `.claude/skills/_shared/claim_carry.py`.

(a) If Step 1 found a retiring handoff for this scope, read its body and call
...
`claim_carry.diff_dropped(prior_claims, successor_text)`. If it returns any
id, hard-FAIL this step - mirroring the Step 4 filename-validation
...
- `payload.step_results`: object with keys `step_0`, `step_1`, `step_2`, `step_2_5`, `step_2_6`, `step_3`, `step_4`, each value `PASS` / `SKIPPED` / `FAIL`. `step_2_5` (the readiness gate) must be `PASS` - it is mandatory and never `SKIPPED`. Under `terminal_state: retired-outright`, steps `step_2`, `step_2_6`, `step_3`, and `step_4` are `SKIPPED`.
