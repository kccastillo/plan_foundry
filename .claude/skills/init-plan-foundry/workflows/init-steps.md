...
Idempotent ten-step bootstrap. Each step PASSes (the operation landed), SKIPPEDs (already present, no action needed), or FAILs (operation could not complete; report and continue unless explicitly fatal).
...
`copy_bundle_managed` copies the four bundle-managed top-level subdirs (`skills/`, `agents/`, `commands/`, `hooks/`) from bundle into target. It never deletes from target. Returns a `CopyReport`.
...
## Step 4: Ensure current-month LOG file

Read `Workbench/` and look for any file matching `*_LOG_YYYYMM.md` where `YYYYMM` is the current year-month. If present, SKIPPED. If absent, create `Workbench/YYYYMMDDHHMI_LOG_YYYYMM.md` (using current timestamp) with this body:
...
**Out of scope:** The bundle-injected `AskUserQuestion` deny is NOT removed by `plan-foundry-uninstall`. Whether uninstall should strip bundle-injected deny entries is a separate design question (see PLAN-AH2 §Out of scope).
...
- `payload.step_results`: object with keys `step_0` through `step_9`, each value `PASS` / `SKIPPED` / `FAIL`.
