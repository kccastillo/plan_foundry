...
2. **Capture, don't prosecute.** Write the observation into `Workbench/wip/plan_foundry-handoff-<YYYY-MM-DD>/` as a plain markdown note (one file per finding, or grouped into a small package with its own `INDEX.md` if multiple findings accumulate). Use frozen `RESEARCH_*`-style filenames or `HICCUP-*.md` — these are not project artefacts and must not enter the project's `Workbench/INDEX.md` or PLAN ID ledger.
3. **Carry on.** If the symptom blocks immediate work, apply the smallest in-conversation workaround and note it in the handoff file. Resume this project's task.
4. **Hand off upstream.** Periodically (or at session-end), transfer the `plan_foundry-handoff-*` folder to the plan_foundry repo at <https://github.com/kccastillo/plan_foundry> for triage there. The plan_foundry side decides whether each finding becomes a bug, an enhancement PLAN, or retrospective input. Once transferred, the local handoff folder can be retired.
