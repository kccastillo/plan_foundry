#!/usr/bin/env python3
"""
test_init_foundry.py — Smoke test for the init-plan-foundry skill.
...
Run from the repo root:
    python3 .claude/skills/init-plan-foundry/scripts/test_init_foundry.py
...
SKILL_ROOT = PLUGIN_ROOT / "skills" / "init-plan-foundry"
...
def test_static_artifacts():
    ...
    expect(skill_md.exists(), "init-plan-foundry SKILL.md exists")
    expect(workflow_md.exists(), "init-plan-foundry workflows/init-steps.md exists")
    expect(template_md.exists(), "init-plan-foundry templates/claude-md-stub.md exists")
...
def simulate_init_foundry(target_dir: Path, operating_rules_content: str) -> dict:
    ...
    if not claude_md.exists():
        ...
        if start_count == 0 and end_count == 0:
            ...
            block = (
                f"\n{SENTINEL_START}\n"
                "<!-- WARNING: content between these markers is managed by the plan-foundry init-plan-foundry skill. Re-running the skill replaces everything between the markers with the current operating-rules.md from the plugin. Do not hand-edit between markers — edits will be lost on re-run. To customise, edit the plugin's operating-rules.md or move the content outside the markers. -->\n\n"
