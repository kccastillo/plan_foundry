...
Operator data untouched. `Workbench/` (PLAN files), `Retired/` (closed-out artefacts), the monthly LOG, and anything under `.claude/` that isn't a bundle-managed dir - never touched. If the operator wants to delete them too, that's a manual `rm -rf` they can run themselves.
...
<constraints>
- Never delete `Workbench/`, `Retired/`, monthly LOG files, or anything under `.claude/` other than the four bundle-managed dirs + the version pin file.
