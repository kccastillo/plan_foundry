...
Note: `Retired/` and `Workbench/.heartbeat/` are bundle-managed gitignore entries (init added them), but the directories themselves are operator data — they remain on disk. Removing the gitignore line just lets the operator track them in git again if they want.
