...
Confirm `plan_path` is absolute. If relative, prepend the repo root path so the Routine - the server-side hourly cron trigger created in Step 3 - reads the correct file regardless of session working directory.
...
that trigger_id, THEN surface the message:
  ...
  - pipeline_phase is drafted AND audit_state.last_outcome is revision_needed AND
    triaged_human_items contains real_judgement_call entries:
      "PLAN-<plan-id> blocked on real_judgement_call items: [items]. Awaiting
       human input. Autonomous loop Routine terminated."
...
- The Routine defers to any active interactive session holding the orchestrator lock. If another session holds the lock, the Routine exits without advancing or writing anything to frontmatter, because an interactive session takes precedence over a Routine firing.
- `total_loop_attempts` is incremented on every non-deferred firing, and a deferred firing, where the lock is held, does not increment the counter.
- The `total_loop_attempts` counter is in `OWNED_FIELDS` (orchestrator_state_guard.py) so the snapshot/restore guard protects that field. The counter is not zeroed on `drafted`-revert, because its purpose spans resets to maintain the IAL guard (IAL: infinite agentic loop, the non-termination failure mode the ceiling exists to bound) across auto-fix cycles.
- The IAL ceiling of 10 attempts bounds the composition of hourly cron firings and the per-session audit-iteration ceiling `MAX_ITERATIONS` end-to-end.
- The within-2-of-ceiling warning at `>= 8` attempts is the cost-runaway mitigation.
- The re-grounding check at Routine startup re-reads the PLAN from disk to verify state before proceeding, which is the artefact-degradation mitigation.
