...
description: Discover all handoff files at session start (glob `Workbench/HANDOFF-*.md` — legacy `HANDOFF-NEXT-SESSION.md`, old-grammar `HANDOFF-<scope>.md`, and new-grammar `HANDOFF-YYYYMMDD-hhmm-<slug>.md`; per PLAN-AF6 D2 the glob intentionally spans all grammars; note that `HANDOFF-NEXT-SESSION.md` is a legacy read/retire-only form — it is no longer written as a new handoff as of PLAN-AH0 D1) and surface them as structured orientation (four-part content contract: session roadmap, agentic & model plan, next-handoff trigger, blocking decisions/constraints). When more than one handoff exists, lists them (scope + last-modified + title) and asks which to surface/resume; a single handoff surfaces directly. Retire decision is deferred to session end — rehydration leaves the handoff on disk; retire fires at session end via handoff-next-session (either updated-into-successor or retired-outright). An explicit unprompted operator "retire now" request is honoured as an override, with the same AA2 post-condition verification. No-op when no handoff is present. Trigger phrases: "rehydrate handoff", "catch me up", "what's pending", "what's the state", "resume session", "list handoffs".
---
...
- Skill returned a `<pipeline-result>` block with structured payload including: `handoffs_found: [...]`, `selected_scope`, `retired: bool`, `retire_deferred: bool`, and `retired_path: string | null`.
</success_criteria>
