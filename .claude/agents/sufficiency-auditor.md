...
Exception conditions: the PLAN file is unreadable, referenced linked_inputs files are missing, the PLAN is structurally malformed (no Steps section, no Verification section), or the review process itself fails (e.g. the shared `_shared/plan-safe.md` reference is missing, which would block plan-safety-auditor next anyway).
