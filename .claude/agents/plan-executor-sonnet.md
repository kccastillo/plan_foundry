...
skills: [execute-plan]
description: "Background subagent that runs the execute-plan skill against a checked PLAN. Sonnet variant, used for PLANs whose `assigned_to: sonnet` indicates Haiku will not cope (some judgement required, larger context, more reasoning). Per parent PLAN 202605011400 decisions 8 (assignments are guidelines), 17 (skills preload), 18 (background), with PLAN-driven model selection."
---

# plan-executor-sonnet

This agent holds the same role as `plan-executor` (haiku) at an upgraded tier. Inputs: `{plan_path: string}`. Outputs: `{outcome: enum[success, revision_needed, exception], payload: {outcome_subtype: enum[done, partially-complete, blocked, needs-revision], executor_notes: string, files_modified: list}, diagnostics}`.
