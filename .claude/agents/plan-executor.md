...
**Default execution tier is Sonnet** (recalibrated 2026-07-04). Haiku is retired as a plan-execution tier — Sonnet 5 is the floor for running a checked PLAN; the reliability gain over Haiku is worth the cost for execution work. `assigned_to: haiku`/empty/absent all route here (Sonnet); `assigned_to: sonnet` routes to the identical `plan-executor-sonnet`; `assigned_to: opus` routes to `plan-executor-opus` for design-heavy execution. Haiku is still used elsewhere (retire, log-summary, ad-hoc search), just not for execution.
