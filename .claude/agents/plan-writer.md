...
Before finalising a spec, count the spec's deliverables that involve discrete units subject to known capacity thresholds (MCP tools registered, schema tables, context-window slices). Consult `plugins/plan-foundry-core/skills/_shared/capacity-thresholds.md` for the current threshold registry.
