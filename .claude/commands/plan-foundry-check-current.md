...
3. Compares the project pin to the remote HEAD.
...
- If `sync_incomplete`: a previous sync started and did not finish - report the sha that sync was moving to (`sync_incomplete.target_sha`) and surface "run /plan-foundry-sync again to complete it". `check_current.py` decides this case ahead of any network call, so the status can be reported even when the pin would otherwise compare as current.
