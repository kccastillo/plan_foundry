...
3. Copies bundle-managed paths into the project (`skills/`, `agents/`, `commands/`, `hooks/`), and performs receipt-backed quarantine of files no longer shipped upstream rather than deleting them outright (PLAN-AH7).
4. Refreshes the version pin at `.claude/.plan-foundry-bundle-version` (records bundle SHA, tag, sync timestamp).
5. Reports: previous SHA -> new SHA, files copied, files unchanged, project additions preserved, stale-in-target list (bundle files no longer upstream, which survive in the target but are worth flagging).
