...
A **deliberative** planning harness for Claude Code. You write down what you're trying to do; the harness audits it, helps you fix it before any code is written, then executes it under supervision and records what happened. Portable copy-paste bundle — clone once, symlink into any target repo, update via `git pull`.

## Why you might want this

Coding agents are very good at producing plausible-looking output very quickly. That is a problem when "plausible-looking" and "actually correct" diverge — which they do, often, on anything beyond a small well-specified change.

The usual failure modes:

- **The agent confidently builds the wrong thing.** The prompt was ambiguous, the agent picked an interpretation, and you only notice when the code lands.
- **Hallucinated APIs, columns, enum values.** The agent wrote SQL against a table column that doesn't exist, or imported a symbol that was never exported.
- **Decisions disappear into chat scrollback.** You agreed on an approach three days ago; today nobody — human or agent — can find the reasoning.
- **Rework cascades.** The agent finishes the code, you spot a structural problem, and now there's no clean way to back out partway because the work wasn't decomposed up front.
- **"Done" that isn't.** Boxes get ticked optimistically; verification was vibes-based.

plan_foundry exists because the cost of building the wrong thing is almost always higher than the cost of thinking carefully first — and because the discipline of *writing the plan down, having it audited, then executing it* is exactly the kind of structure agents are bad at imposing on themselves but good at following once it's imposed.
...
- Throughput. If you want an agent to ship code as fast as possible, reach for a [ralph loop](https://ralphloop.sh/), a multi-agent swarm, or Claude Code's native Agent Teams. plan_foundry is deliberately sequential and human-in-the-loop. Parallelism doesn't help you think harder.
