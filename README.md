...
Turns Claude Code planning and execution into a state machine on disk - drafted,
audited, executed, and verified as files in your repo, not held in the model's
head between turns.
...
| **INPUT** | A context artefact - findings, a data drop, survey results, or a strategic note captured mid-flight. `type: input`. Written via `write-input`; consumed and retired once integrated into a PLAN. Files under the older `ADVICE-*` and `RESEARCH-*` names are the same thing and stay readable. |
...
| **`write-input`** | Skill (verb). Records an INPUT file and auto-clears any PLAN blocked waiting on it. |
...
frontmatter, and moves through the fixed lifecycle enum described in "How it
works" above. The `plan-pipeline` skill is the orchestrator - it reads
frontmatter, decides which phase fires next, and dispatches the right sub-skill.
The state machine lives on disk, not in the model's head, so re-entry is
idempotent. `write-plan` authors a PLAN and enforces the substrate-verification
preflight; `execute-plan` runs an already-checked PLAN end-to-end and records
the outcome.
...
`write-input` records inputs - findings, data drops and strategic notes - as their
own files in `Workbench/`; PLANs blocked on a missing input auto-clear when it
lands. An input is marked integrated and retired automatically when a PLAN that
lists it retires.
...
Invoking it does not commit you to all of that. A proportionality gate runs first
and asks how much of the machinery this particular problem needs. It assesses
three things - whether the requirement is agreed, whether the mechanism is forced,
and whether the change is reversible and how wide it reaches - then offers four
rungs with a recommendation:
...
| `full arc` | The eight phases, both risk gates, then the audit loop and the pipeline. |
...
recurring rule earns it, and removal when content is no longer load-bearing. Its
scope is CLAUDE.md, ARCHITECTURE.md, CONTEXT_CONSTITUTION.md and the durable
`_shared/*.md` helpers.

### Building skills: `skill-standard.md` + `write-skill` + `audit-skills`

Three artefacts with strictly separated jobs, and all three are for your own skills
as much as for this bundle's.

`.claude/skills/_shared/skill-standard.md` is the definition and has no behaviour. It
says what a SKILL.md must state, which scaffolding is worth keeping and which is
working around a weakness the model no longer has, what the frontmatter contract is,
and what a description has to do to fire reliably.

`write-skill` scaffolds exactly one skill against it and then proves it triggers,
measuring the new description against a set of prompts that must fire it and a set of
...
installed set costs in context on every turn. It reports and never patches. Where a
skill belongs to a bundle rather than to you, it raises a request instead of proposing
an edit that the next sync would destroy.
...
(the agent reads this on first contact). The install copies bundle content into
...
receipt at `.claude/.bundle-receipts/plan_foundry.files` - namespaced by bundle
identity so a second bundle installed alongside plan_foundry cannot overwrite this
one's record of what it wrote. Idempotent - safe to re-paste.
...
Read it before your first sync onto a new version.
