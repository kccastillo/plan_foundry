...
**Problem:** Installing into a new repo should be one paste-prompt. Updates should be one command. Removal should be one command. All three must work inside Claude Code sessions whose filesystem write surface is the target repo only (mobile, web, sandboxed desktop) — network is available, but `~/.claude/...` isn't reachable.

**Solution (AC6, 2026-05-19):** Bundle source is the public URL, fetched on demand into a transient `<target>/.plan-foundry-tmp/` (gitignored). `/init-plan-foundry` clones, copies, scaffolds, and deletes the tmp. `/plan-foundry-sync` does the same minus scaffolding. `/plan-foundry-uninstall` removes everything plan_foundry added (offline; idempotent). `/plan-foundry-check-current` queries the remote via `git ls-remote` and compares to the project's pin.
...
The agent will (1) `git clone --depth=1 https://github.com/kccastillo/plan_foundry .plan-foundry-tmp/`, (2) run `python3 .plan-foundry-tmp/.claude/skills/init-plan-foundry/lib/run_install.py`, (3) `rm -rf .plan-foundry-tmp/`. The install copies bundle content into `.claude/{skills,agents,commands,hooks}`, scaffolds `Workbench/` and `Retired/`, seeds the current-month LOG, updates `.gitignore`, inlines operating rules into `CLAUDE.md`, and records the bundle commit SHA at `.claude/.plan-foundry-bundle-version`. Idempotent — safe to re-paste.

After install, **restart Claude Code** so the freshly-copied skills register with the harness.

See [BOOTSTRAP.md](BOOTSTRAP.md) for the canonical procedure (the agent reads this on first contact).
...
Sync clones the public repo into `<target>/.plan-foundry-tmp/`, overwrites bundle-managed files with the latest content, refreshes the version pin, and deletes the tmp clone. Never deletes from the target — project additions under `.claude/skills/`, `.claude/agents/`, etc. survive. Files renamed or removed upstream are reported so you can clean them manually. Project-local files outside the four bundle-managed dirs (`settings.local.json`, `plan-foundry.config`, custom scripts) are never touched.
...
Compares your `.plan-foundry-bundle-version` to the remote HEAD via `git ls-remote`.
...
Removes the four bundle-managed dirs, the version pin, the bundle `.gitignore` entries, and the CLAUDE.md sentinel block. Leaves `Workbench/`, `Retired/`, monthly LOG, and project-local `.claude/` files (`settings.local.json`, `plan-foundry.config`) untouched — those are operator data, not bundle code. Offline; idempotent.
