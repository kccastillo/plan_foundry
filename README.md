...
**Solution:** `/init-plan-foundry` (or `Skill("init-plan-foundry")`) copies the bundle's `.claude/{skills,agents,commands,hooks}` into the target, scaffolds `Workbench/` and `Retired/`, updates `.gitignore`, inlines operating rules into `CLAUDE.md`, and records the bundle version pin. Idempotent. `/plan-foundry-sync` pulls subsequent bundle updates into the project (overwrites bundle-managed files, preserves project additions). `/plan-foundry-check-current` reports two-tier currency: bundle vs `origin/main`, AND this project's pinned version vs the bundle.
...
(Or invoke `Skill("init-plan-foundry")` in a Claude Code session running inside the target repo, or say *"set up plan_foundry"*.) The skill copies the bundle's `.claude/{skills,agents,commands,hooks}` into the target as real per-project content, scaffolds `Workbench/` and `Retired/`, seeds the current-month LOG, updates `.gitignore` (gitignoring bundle-managed paths but tracking project-local files), inlines operating rules into the target's `CLAUDE.md`, and records the bundle commit SHA at `.claude/.plan-foundry-bundle-version`. Idempotent — safe to re-run. Legacy AC3 symlink installs are migrated in-place.

After install, **restart Claude Code** so the project-local `.claude/` directory is registered.

Update the bundle clone (does NOT auto-propagate to projects):

```
cd ~/.claude/plan_foundry && git pull
```

Pull the latest bundle into a project:

```
/plan-foundry-sync
```

Sync overwrites bundle-managed files; it never deletes from the target, so project additions under `.claude/skills/`, `.claude/agents/`, etc. survive. Project-local files outside the four bundle-managed dirs (`settings.local.json`, `plan-foundry.config`) are never touched.

Check both tiers of currency (bundle vs upstream; project vs bundle) with `/plan-foundry-check-current`.
