#!/usr/bin/env bash
# ascii-exempt (D18): the INDEX-freshness check contains literal mojibake
# byte sequences as DATA - it normalises corrupted em dashes and quotes so
# the committed-vs-fresh comparison does not flake. Sweeping them would
# break the repair. Open question for W0.1 follow-up: now that build_index
# reads git output with an explicit encoding, is this repair still needed?
#
...
#   - Hook line endings (every .claude/hooks/ file is CR-free with eol=lf pinned)
#   - audit-foundry.py exits clean (no error-severity findings)
#   - Foundry invariants (executable ARCHITECTURE.md register assertions)
...
def strip_md(s):
    ...
    # The Alerts section carries wall-clock-relative strings ("last commit was
    # Nh ago"). strip_json already zeroes the volatile alert lists; mask the
    # markdown counterpart the same way so the check does not flake when CI runs
    # minutes after the INDEX was committed.
    s = re.sub(r"## Alerts.*?(?=\n## |\Z)", "## Alerts\n<masked>\n", s, flags=re.DOTALL)
...
Checks:
  ...
  - update-workbench-index tests: build_index
  - write-plan tests: migrate_plan_ids
  - write-plan tests: next_id
  - _shared tests: push_policy
  - _shared tests: push_guard
  - _shared tests: orchestrator_lock
  - _shared tests: orchestrator_state_guard
  - _shared tests: resume_preflight
  - _shared tests: auditor_schema_code_pattern
  - _shared tests: validate_artefact_filename
  - plan-pipeline tests: model_budget
  - plan-pipeline tests: gate_halt_routing
  - handoff-next-session tests: readiness_gate_wiring
  - audit-haiku-safe tests: sizing_wiring
  - ideate tests
  - INDEX freshness
  - pre-commit hook syntax
  - promote.sh syntax
  - dogfood telemetry hook registered
  - hook line endings (LF + eol=lf pinned)
  - audit-foundry clean
  - utf8 stdout guard
  - foundry invariants
...
run_check "update-workbench-index: build_index" \
    python3 .claude/skills/update-workbench-index/lib/test_build_index.py

run_check "update-workbench-index: project_context_inputs" \
