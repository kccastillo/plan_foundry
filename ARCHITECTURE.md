...
- A bundle refuses to write a path it did not write last (PLAN-AK5), rather than picking a winner between it and whatever a sibling bundle wrote there. The install receipt is namespaced by bundle identity (`.claude/.bundle-receipts/<bundle>.files`, superseding the shared-filename `.claude/.plan-foundry-bundle-files`, which stays readable as a legacy fallback), because two bundles descended from this lineage both wrote the same shared filename and each sync overwrote the other's ownership record - the namespacing is what makes "these bytes are not the ones we wrote" answerable at all, not a stronger identification of which sibling owns a path. `sync` cannot prove another bundle owns a colliding path, only that its own receipt does not vouch for what is there now; it reports what it knows and refuses that one write rather than naming a culprit it cannot identify, while the rest of the run completes. This is what stops two bundles sharing a directory - `.claude/hooks/`, most visibly - from clobbering each other silently, the two-bundles-same-hook-path defect this invariant closes.

**Verified by:** `plan-foundry-sync/lib/test_sync.py` asserts the shipped contract names this bundle, that a foreign `_shared/` diverts helper loading to the clone and the sync still completes, that the ordinary path is untouched, and that quarantine skips modified files while still moving unmodified ones - and, per PLAN-AK5, that a write to a path the receipt does not vouch for is refused and reported rather than applied, that an ordinary upgrade of a path the receipt does vouch for still proceeds, and that a resolved refusal self-clears from the standing conflicts file. Companion checks live in `plan-foundry-uninstall/lib/test_uninstall.py` and `plan-foundry-check-current/lib/test_check_current.py`.
