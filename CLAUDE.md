...
- **Maintenance (human-triggered):** `lessons-learned` (post-PLAN retrospective), `test-foundry` (run harness), `foundry-log` (log-month rotation), `init-plan-foundry` (bootstrap a target repo), `plan-foundry-check-current` (is the bundle current with origin), `plan-foundry-sync` (copy bundle from clone into target's `.claude/`), `plan-foundry-uninstall` (reverse of init-plan-foundry).
