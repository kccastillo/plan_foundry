...
- **Writing style:** [.claude/skills/_shared/writing-style.md](.claude/skills/_shared/writing-style.md) is the single source for how written output should read - prose tells to watch for, and the word/verb/sentence/structure rules. Read it before any longform output. Its character-set rule is scoped to D9 (git boundary only); prose em dashes stay deliberate.
...
**PLAN identity:** active PLANs use the `PLAN-[A-Z][A-Z][0-9]` scheme (AA0-ZZ9). Historical `PLAN-NNN` IDs in `Retired/` and the LOG are frozen. Full policy in [plan-conventions.md section PLAN Identity Policy](.claude/skills/write-plan/references/plan-conventions.md). Allocator is `.claude/skills/write-plan/scripts/next_id.py`.
