...
`.claude/_foundry_log.jsonl` — configurable via the `foundry-keeper.logPath` setting if the plugin settings mechanism supports custom keys in the future, otherwise via `.claude/plan-foundry.config` `logPath` key.
