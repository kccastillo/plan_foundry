...
for the eight-phase ideate cadence (Phase 4 onward).

Phases 1–3 are conversational and leave no disk state. This module
is only active from Phase 4 onward.

Reference: plugins/ideate/skills/ideate/references/phase-transitions.md
"""
...
VALID_TRANSITIONS: dict[str | None, set[str]] = {
    None: {"spec_draft", "exited_early"},
    "": {"spec_draft", "exited_early"},
    "clarify": {"survey", "spec_draft", "exited_early"},
    "survey": {"converge", "clarify", "spec_draft", "exited_early"},
    "converge": {"spec_draft", "exited_early"},
    "spec_draft": {"self_critique", "exited_early"},
    "self_critique": {
        "spec_refine",
...
def advance_phase(plan_path: Path, current: str, next_phase: str) -> None:
    ...
    # Increment counters based on destination phase
