...
_PLUGINS_ROOT = _THIS_DIR.parent.parent.parent.parent.resolve()
_SEVERITY_LIB = _PLUGINS_ROOT / "plan-foundry-core" / "skills" / "plan-pipeline" / "lib"
...
def test_advance_phase_valid_transition():
    """advance_phase writes the new phase to frontmatter and increments counter."""
    with tempfile.TemporaryDirectory() as tmp:
        plan_path = _write_plan(Path(tmp))

        # Transition spec_draft → self_critique (should increment self_critique counter)
        state.advance_phase(plan_path, "spec_draft", "self_critique")

        s = state.read_ideate_state(plan_path)
        assert_eq(s["ideate_phase"], "self_critique", "phase after advance")
        assert_eq(s["ideate_iteration_count"]["self_critique"], 1, "counter after first advance")
