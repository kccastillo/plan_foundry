...
5. **Research-prompt template:** Every dispatch reads `plugins/plan-foundry-core/skills/_shared/research-prompt-template.md` for the mandatory sub-questions (public-API surface, prior-art citations, lean-reversibility, verdict line). These sub-questions are included verbatim in every research-bot prompt.
...
**Purpose:** Structured self-critique gate. Produces a critique JSON against the Phase 4 spec. Presents findings to the human using the severity-surface pattern (via `lib/render_critique.py`). Self-Critique focuses on **structural omissions** — missing sections, missing details, internal inconsistency — and is distinct from audit-sufficiency (invariant violations) and audit-haiku-safe (mechanical safety). See [`plan-foundry-core/skills/_shared/audit-stages.md`](../../../../plan-foundry-core/skills/_shared/audit-stages.md) for the three-tier focus distinction.
