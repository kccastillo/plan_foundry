...
**init-foundry sentinel-marker awareness (host CLAUDE.md).** When the audited CLAUDE.md contains a `<!-- plan-foundry:init-foundry:start -->` ... `<!-- plan-foundry:init-foundry:end -->` block placed there by the `init-foundry` skill, the audit treats the content between the markers as plugin-managed:
