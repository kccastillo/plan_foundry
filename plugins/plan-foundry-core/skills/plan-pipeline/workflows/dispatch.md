...
1. Invoke `Skill("ideate:ideate", request_or_existing_path)` directly in the parent session. **Never via a subagent.** (Plugin-qualified — the ideate skill lives in the `ideate` plugin; bare `Skill("ideate")` fails across plugin boundaries.)
...
```python
# Modules are in plugins/plan-foundry-core/skills/plan-pipeline/lib/
...
```bash
python "${CLAUDE_PLUGIN_ROOT:-plugins/plan-foundry-core}/skills/plan-pipeline/lib/build_brief.py" \
...
3. Branch:
   - `success` → commit + push (`plan-pipeline: retired <plan-filename>`). Surface "Pipeline complete: <plan-filename> retired." Continue to sub-step 4 (drift check).
   - `exception` → kanban halt. Commit + push WIP. Surface diagnostics using the kanban-halt surface (branch 5). Return.

4. **Doc-drift check** (after successful retire, auxiliary). Read the just-completed PLAN's `files_touched` frontmatter. If any path matches any of the following patterns, foreground-dispatch `Skill("maintain-claude-md")` with no arguments — it audits the live state of CLAUDE.md and ARCHITECTURE.md against the current codebase (AGENT_RULES.md was dissolved 2026-05-14 per Option Y of ADVICE-003; its meta-policy lives in CLAUDE.md now):
   - `plugins/*/skills/*` (any skill change)
   - `plugins/*/commands/*` (any slash command change)
   - `plugins/*/skills/_shared/*` (invariants / plan-safe live here — ARCHITECTURE.md driver)
   - `plugins/*/agents/*` (executor capability boundaries — ARCHITECTURE.md driver)
   - `CLAUDE.md` itself
