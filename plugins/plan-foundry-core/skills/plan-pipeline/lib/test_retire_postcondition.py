...
The plan-pipeline orchestrator's §4F retire step must independently verify the
...
RETIRE_WORKFLOW = REPO_ROOT / "plugins/plan-foundry-core/skills/retire/workflows/retire-file.md"
RETIRE_SKILL = REPO_ROOT / "plugins/plan-foundry-core/skills/retire/SKILL.md"
PLAN_RETIRER = REPO_ROOT / "plugins/plan-foundry-core/agents/plan-retirer.md"
...
def test_dispatch_4F_has_post_condition_check():
    """plan-pipeline dispatch §4F must mention post-condition verification."""
    text = _read(DISPATCH)
    # Locate §4F
    assert "### 4F. `complete`" in text or "4F. complete" in text, "§4F section missing"
    ...
    assert "post-condition" in text.lower() or "post_condition" in text.lower(), (
        "§4F is missing post-condition language"
    )


def test_dispatch_4F_mentions_orchestrator_independent_verify():
    """The §4F prose must establish that the orchestrator verifies independently of the subagent."""
    text = _read(DISPATCH)
    ...
    assert "retired/" in lower, "§4F must reference the Retired/ destination"
    # Either "stat", "exists", or "verify" must appear in the §4F region near the post-condition keyword
    assert any(keyword in lower for keyword in ("stat the source", "stat the destination", "assert it", "independently verify")), (
        "§4F must specify independent filesystem verification"
    )
...
def test_retire_workflow_updates_log_status_table():
    """retire workflow must include the D-γ fix: update monthly LOG Status Table on retire."""
    text = _read(RETIRE_WORKFLOW)
    lower = text.lower()
    # Required signals for LOG-update step
    assert "monthly log" in lower or "_log_" in lower or "log status table" in lower or "status table" in lower, (
        "retire workflow is missing the LOG-update step (D-γ fix)"
    )
    assert "d-γ" in lower or "d-gamma" in lower or "hiccup-log-supplement" in lower, (
        "retire workflow LOG-update step is missing the D-γ rationale reference"
    )


def test_retire_skill_success_criteria_mentions_log_update():
    """retire SKILL.md success_criteria must include the D-γ LOG-update item."""
    text = _read(RETIRE_SKILL)
    block = text.split("<success_criteria>", 1)[1].split("</success_criteria>", 1)[0]
    lower = block.lower()
    assert "log status table" in lower or "monthly log" in lower or "d-γ" in lower or "d-gamma" in lower, (
        "success_criteria is missing the LOG-update requirement (D-γ fix)"
    )
