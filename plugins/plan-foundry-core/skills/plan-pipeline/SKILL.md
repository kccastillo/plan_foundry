...
Ideation is interactive and runs in the parent session only — invoke `Skill("ideate:ideate")` directly (plugin-qualified — bare `Skill("ideate")` fails across plugin boundaries), never via a subagent. The structural safeguard is the absence of an `ideate-runner` agent file. (Decision 10.)
Audit failure is normal. The lifecycle supports `revision_needed` → Spec-Refine → re-audit gracefully; do not hide audit findings or force a clean pass on iteration 1. See ARCHITECTURE.md §Productive audit failure.
...
- Never run `ideate` as a subagent. Invoke `Skill("ideate:ideate", ...)` in the parent session (plugin-qualified; the ideate skill lives in the `ideate` plugin, not `plan-foundry-core`).
