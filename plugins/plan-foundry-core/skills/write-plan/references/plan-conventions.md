...
**Allocator.** `plugins/plan-foundry-core/skills/write-plan/scripts/next_id.py` is the canonical allocator. Source of truth: LOG primary (Status Table column 1), filesystem secondary (warn-and-union on disagreement). No cache. Read-only contract — not idempotent across calls. See PLAN-AA1's design lineage (research-validated leans across event-sourcing canonical pattern, Postgres `nextval` template, Jira/Linear/GitHub ID conventions).
...
2. `.claude-plugin/marketplace.json` top-level `push_policy` field — project default.
...
**Project default:** The `plan_foundry_dev` repo sets `push_policy: manual` in `.claude-plugin/marketplace.json` because it runs under a harness with commit-scope authorisation controls. New consumers without a `marketplace.json` setting also default to `manual` (safest for first-use). Set `push_policy: auto` in `marketplace.json` when the environment allows unrestricted pushes, or per-PLAN for individual plans that need immediate remote propagation.

**Implementation:** `plugins/plan-foundry-core/skills/_shared/push_policy.py` — function `get_push_policy(plan_path, repo_root=None) -> str`.
...
See `plugins/plan-foundry-core/skills/audit-haiku-safe/workflows/audit-haiku-safe-steps.md` (Step 4a: Substrate fidelity check) and `plugins/plan-foundry-core/skills/write-plan/workflows/write-plan.md` (Step 0: Substrate-verification preflight) for the enforcement sites.
...
**Forbidden patterns (when unannotated):** `/tmp/`, `/dev/null`, `bash -c`, `test -[a-zA-Z]`, `> /dev/`, `2>/dev/null`, `&&` in compound commands. See `plugins/plan-foundry-core/skills/_shared/plan-safe.md` "Platform portability" section for the full list with rationale and portable alternatives.
...
If any discrete deliverable count (MCP tools, schema tables, context-window slices) exceeds **0.8×** a threshold in `plugins/plan-foundry-core/skills/_shared/capacity-thresholds.md`, the PLAN's Context section MUST acknowledge the brushing and note that a research bot was dispatched to confirm the threshold's current relevance.
...
- **F1 — Research-anchor:** Survey Phase 2.B auto-dispatches one research bot per Real-judgement-call cluster when ≥2 such clusters exist. Each bot uses the sub-questions in `plugins/plan-foundry-core/skills/_shared/research-prompt-template.md`.
...
**Source of truth:** `plugins/ideate/skills/ideate/workflows/cadence-phases.md` § Phase 2 (Survey), § Phase 5 (Self-Critique), § Phase 7 (Cross-Spec-Reconcile).
**Structural tests:** `plugins/ideate/skills/ideate/lib/test_cadence_structure.py` (7 tests; run via `python -m pytest`).
