...
# PLAN file pattern — supports both legacy timestamp-prefix (pre-2026-05-13) and
# new type-prefixed sequential (PLAN-NNN_, ADVICE-NNN_, RESEARCH-NNN_).
PLAN_PATTERN = re.compile(r"^(?:\d{12}_(?:PLAN|ADVICE|RESEARCH)_|(?:PLAN|ADVICE|RESEARCH)-\d{3}_)")
...
def load_audit_history(workbench_dir: Path, plan_id: str) -> list:
    ...
    Returns a list of audit dicts, sorted by iteration ascending.
    """
    ...
    for audit_file in sorted(audit_dir.iterdir()):
        ...
        try:
            with open(audit_file, encoding="utf-8") as f:
                data = json.load(f)
            data["_iteration"] = int(m.group(1))
            data["_file"] = str(audit_file)
            results.append(data)
        except (json.JSONDecodeError, OSError):
            pass

    return sorted(results, key=lambda x: x.get("_iteration", 0))
...
def compute_alerts(plans: list, workbench_dir: Path) -> dict:
    """
    Compute all ten alert categories.
    ...
    for plan in plans:
        ...
        # recurring_blockers: same error fingerprint in 2+ audit iterations
        audit_history = load_audit_history(workbench_dir, pid)
        if len(audit_history) >= 2:
            # Gather error fingerprints by iteration
            iter_fps = []
            for audit in audit_history:
                ...
                iter_fps.append(fps)
            # Find fingerprints that appear in 2+ consecutive iterations
            for i in range(1, len(iter_fps)):
                recurring = iter_fps[i] & iter_fps[i - 1]
                if recurring:
                    alerts["recurring_blockers"].append({
                        "plan_id": pid,
                        "detail": f"Error fingerprint(s) recur across audit iterations {i} and {i+1}: "
    ...
    if audit_dir.exists():
        for audit_file in audit_dir.iterdir():
            if not audit_file.name.endswith(".json"):
                continue
            # Strip -<iter>.json
            m = re.match(r"^(.+)-\d+\.json$", audit_file.name)
            if m:
                file_plan_id = m.group(1)
                if file_plan_id not in plan_ids:
...
def render_markdown(
    ...
    if retired_dir.exists():
        retired_plans = sorted(
            [f for f in retired_dir.iterdir() if f.name.endswith(".md") and PLAN_PATTERN.match(f.name)],
            key=lambda x: x.stem,
            reverse=True,
        )[:10]
        if retired_plans:
            ...
            for rp in retired_plans:
                try:
                    rtext = rp.read_text(encoding="utf-8")
                    rfm = _parse_frontmatter(rtext)
                    rtitle = rfm.get("title", "(no title)")
                except OSError:
                    rtitle = "(unreadable)"
                lines.append(f"| {rp.stem} | {rtitle} |")
        else:
            lines.append("_No retired PLANs found._")
    else:
        lines.append("_Retired/ directory does not exist._")
    lines.append("")
